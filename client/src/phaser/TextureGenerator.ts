...
/** Extended canvas context with polyfill helpers used by makeSimpleMonster */
interface ExtCtx extends CanvasRenderingContext2D {
...
export class TextureGenerator {
  public static generateAll(scene: Phaser.Scene) {
    if (scene.textures.exists('poring')) return;
    ...
    TextureGenerator.makeSimpleMonster(scene, 'ForestBoar', 48, 36, (ctx) => {
      ctx.fillStyle = '#5c4033'; ctx.fillEllipse(24, 20, 40, 28);
      ctx.fillStyle = '#3e2723'; ctx.fillEllipse(24, 16, 22, 16); // head
      ctx.fillStyle = '#f5f5f5'; // tusks
      ctx.beginPath(); ctx.moveTo(14, 22); ctx.lineTo(8, 30); ctx.lineTo(12, 22); ctx.fill();
      ctx.beginPath(); ctx.moveTo(34, 22); ctx.lineTo(40, 30); ctx.lineTo(36, 22); ctx.fill();
      ctx.fillStyle = '#e53935'; ctx.fillEllipse(18, 16, 4, 4); // eye
      ctx.fillStyle = '#000'; ctx.fillEllipse(18, 16, 2, 2);
    });
    // WildWolf (52x40)
    TextureGenerator.makeSimpleMonster(scene, 'WildWolf', 52, 40, (ctx) => {
      ctx.fillStyle = '#78909c'; ctx.fillEllipse(26, 24, 42, 30); // body
      ctx.fillStyle = '#607d8b'; ctx.fillEllipse(26, 16, 20, 18); // head
      ctx.fillStyle = '#78909c'; // ears
      ctx.fillTriangle(16, 12, 12, 4, 20, 8);
      ctx.fillTriangle(36, 12, 32, 4, 40, 8);
      ctx.fillStyle = '#e53935'; ctx.fillEllipse(20, 17, 4, 4);
      ctx.fillStyle = '#000'; ctx.fillEllipse(20, 17, 2, 2);
      ctx.fillStyle = '#fff'; ctx.fillRect(22, 22, 8, 4); // teeth
    ...
    TextureGenerator.makeSimpleMonster(scene, 'DesertScorpion', 56, 40, (ctx) => {
      ctx.fillStyle = '#d4a017'; ctx.fillEllipse(28, 26, 42, 26); // body
      ctx.fillStyle = '#c9a45c'; // claws
      ctx.fillEllipse(8, 26, 14, 10); ctx.fillEllipse(48, 26, 14, 10);
      ctx.fillStyle = '#f77f00'; // tail segments
      for (let i = 0; i < 4; i++) { ctx.fillEllipse(28 + i * 6 - 6, 10 - i * 4, 8, 8); }
      ctx.fillStyle = '#e53935'; ctx.fillEllipse(18, 18, 5, 5); // eyes
      ctx.fillStyle = '#000'; ctx.fillEllipse(18, 18, 2.5, 2.5);
    });
    // SandGolem (52x60)
    TextureGenerator.makeSimpleMonster(scene, 'SandGolem', 52, 60, (ctx) => {
      ctx.fillStyle = '#c9a45c'; ctx.fillEllipse(26, 38, 44, 40); // body
      ctx.fillStyle = '#d4a017'; ctx.fillEllipse(26, 18, 32, 28); // head
      ctx.fillStyle = '#795548'; // cracks
      ctx.strokeStyle = '#5d4037'; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.moveTo(14, 25); ctx.lineTo(22, 35); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(30, 20); ctx.lineTo(38, 30); ctx.stroke();
      ctx.fillStyle = '#ff7043'; ctx.fillEllipse(20, 18, 7, 7); // eyes glow
      ctx.fillStyle = '#ff3d00'; ctx.fillEllipse(32, 18, 7, 7);
      ctx.fillStyle = '#000'; ctx.fillEllipse(20, 18, 3, 3); ctx.fillEllipse(32, 18, 3, 3);
    });
    // MummyPharaoh (44x60)
    TextureGenerator.makeSimpleMonster(scene, 'MummyPharaoh', 44, 60, (ctx) => {
      ctx.fillStyle = '#ede0c8'; ctx.fillEllipse(22, 36, 36, 48); // wrapped body
      ctx.fillStyle = '#d4b896'; // bandage lines
      for (let y = 18; y < 58; y += 7) { ctx.fillRect(6, y, 32, 3); }
      ctx.fillStyle = '#b5632a'; ctx.fillEllipse(22, 16, 26, 22); // head
      ctx.fillStyle = '#ffd700'; // pharaoh headdress
      ctx.fillRect(8, 6, 28, 8);
      ctx.fillStyle = '#00e5ff'; ctx.fillEllipse(17, 18, 6, 6); ctx.fillEllipse(27, 18, 6, 6); // glowing eyes
      ctx.fillStyle = '#000'; ctx.fillEllipse(17, 18, 3, 3); ctx.fillEllipse(27, 18, 3, 3);
    });
    ...
    TextureGenerator.makeSimpleMonster(scene, 'SkeletonSoldier', 44, 60, (ctx) => {
      ...
      ctx.beginPath(); ctx.moveTo(22, 18); ctx.lineTo(22, 42); ctx.stroke(); // spine
      ctx.beginPath(); ctx.moveTo(10, 26); ctx.lineTo(34, 26); ctx.stroke(); // shoulders
      ctx.beginPath(); ctx.moveTo(10, 26); ctx.lineTo(6, 44); ctx.stroke(); // left arm
      ctx.beginPath(); ctx.moveTo(34, 26); ctx.lineTo(38, 44); ctx.stroke(); // right arm
      ctx.beginPath(); ctx.moveTo(22, 42); ctx.lineTo(14, 58); ctx.stroke(); // left leg
      ctx.beginPath(); ctx.moveTo(22, 42); ctx.lineTo(30, 58); ctx.stroke(); // right leg
      ctx.fillStyle = '#e0e0e0'; ctx.fillEllipse(22, 14, 18, 20); // skull
      ctx.fillStyle = '#ff1744'; ctx.fillEllipse(17, 13, 5, 5); ctx.fillEllipse(27, 13, 5, 5); // red eyes
      ctx.fillStyle = '#000'; ctx.fillEllipse(17, 13, 2.5, 2.5); ctx.fillEllipse(27, 13, 2.5, 2.5);
      ctx.fillStyle = '#90a4ae'; // sword
      ctx.fillRect(36, 18, 4, 28); ctx.fillRect(30, 26, 16, 4);
    });
    // WraithPhantom (44x56)
    TextureGenerator.makeSimpleMonster(scene, 'WraithPhantom', 44, 56, (ctx) => {
      const grad = ctx.createRadialGradient(22, 28, 4, 22, 28, 22);
      grad.addColorStop(0, 'rgba(180,100,255,0.9)'); grad.addColorStop(1, 'rgba(60,0,120,0)');
      ctx.fillStyle = grad; ctx.fillEllipse(22, 28, 42, 48);
      ctx.fillStyle = 'rgba(220,180,255,0.85)'; ctx.fillEllipse(22, 18, 24, 22); // face
      ctx.fillStyle = '#ff1744'; ctx.fillEllipse(16, 17, 6, 7); ctx.fillEllipse(28, 17, 6, 7); // evil eyes
      ctx.fillStyle = '#fff'; ctx.fillEllipse(15, 16, 2.5, 3); ctx.fillEllipse(27, 16, 2.5, 3);
      // Wispy trails
      ctx.strokeStyle = 'rgba(180,100,255,0.5)'; ctx.lineWidth = 3;
      ctx.beginPath(); ctx.moveTo(10, 38); ctx.quadraticCurveTo(8, 52, 14, 56); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(34, 38); ctx.quadraticCurveTo(36, 52, 30, 56); ctx.stroke();
    });
    // DarkGargoyle (56x52)
    TextureGenerator.makeSimpleMonster(scene, 'DarkGargoyle', 56, 52, (ctx) => {
      ctx.fillStyle = '#37474f'; ctx.fillEllipse(28, 34, 44, 36); // stone body
      ctx.fillStyle = '#263238'; ctx.fillEllipse(28, 18, 26, 24); // head
      ctx.fillStyle = '#37474f'; // wings
      ctx.beginPath(); ctx.moveTo(28, 24); ctx.lineTo(4, 14); ctx.lineTo(8, 36); ctx.lineTo(28, 30); ctx.fill();
      ctx.beginPath(); ctx.moveTo(28, 24); ctx.lineTo(52, 14); ctx.lineTo(48, 36); ctx.lineTo(28, 30); ctx.fill();
      ctx.fillStyle = '#ff1744'; ctx.fillEllipse(22, 17, 6, 6); ctx.fillEllipse(34, 17, 6, 6); // red eyes
      ctx.fillStyle = '#fff'; ctx.fillEllipse(21, 16, 2.5, 2.5); ctx.fillEllipse(33, 16, 2.5, 2.5);
      // horns
    ...
    TextureGenerator.makeSimpleMonster(scene, 'FireSalamander', 52, 44, (ctx) => {
      ctx.fillStyle = '#e53935'; ctx.fillEllipse(26, 28, 46, 34); // body
      ctx.fillStyle = '#ff7043'; // flame pattern
      for (let i = 0; i < 5; i++) { ctx.fillEllipse(10 + i * 8, 22, 8, 12); }
      ctx.fillStyle = '#d32f2f'; ctx.fillEllipse(26, 16, 22, 18); // head
      ctx.fillStyle = '#ffeb3b'; ctx.fillEllipse(20, 14, 6, 6); ctx.fillEllipse(32, 14, 6, 6); // eyes
      ctx.fillStyle = '#ff6f00'; ctx.fillEllipse(20, 14, 3, 3); ctx.fillEllipse(32, 14, 3, 3);
      ctx.strokeStyle = '#ff7043'; ctx.lineWidth = 4; // tail
      ctx.beginPath(); ctx.moveTo(46, 30); ctx.quadraticCurveTo(58, 24, 52, 14); ctx.stroke();
    });
    // LavaGolem (56x60)
    TextureGenerator.makeSimpleMonster(scene, 'LavaGolem', 56, 60, (ctx) => {
      ctx.fillStyle = '#212121'; ctx.fillEllipse(28, 38, 50, 44); // dark stone body
      ctx.fillStyle = '#bf360c'; ctx.fillEllipse(28, 20, 34, 30); // head
      // Lava cracks
      ctx.strokeStyle = '#ff6d00'; ctx.lineWidth = 3;
      ctx.beginPath(); ctx.moveTo(14, 32); ctx.lineTo(20, 44); ctx.lineTo(28, 38); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(42, 30); ctx.lineTo(36, 44); ctx.stroke();
      ctx.fillStyle = '#ff6d00'; ctx.fillEllipse(21, 20, 8, 8); ctx.fillEllipse(35, 20, 8, 8); // fire eyes
      ctx.fillStyle = '#fff176'; ctx.fillEllipse(21, 20, 4, 4); ctx.fillEllipse(35, 20, 4, 4);
      ctx.fillStyle = '#ff6d00'; // lava fist
      ctx.fillEllipse(8, 36, 14, 14); ctx.fillEllipse(48, 36, 14, 14);
    });
    // NightmareSteed (64x52)
    TextureGenerator.makeSimpleMonster(scene, 'NightmareSteed', 64, 52, (ctx) => {
      ctx.fillStyle = '#1a1a2e'; ctx.fillEllipse(32, 34, 58, 36); // body
      ctx.fillStyle = '#16213e'; ctx.fillEllipse(20, 18, 22, 26); // head/neck
      ctx.fillStyle = '#0f3460'; // mane
      ctx.fillTriangle(14, 10, 6, 0, 22, 4);
      ctx.fillTriangle(20, 8, 12, -2, 26, 2);
      ctx.fillStyle = '#e040fb'; ctx.fillEllipse(16, 16, 7, 7); ctx.fillEllipse(26, 16, 7, 7); // purple eyes
      ctx.fillStyle = '#fff'; ctx.fillEllipse(15, 15, 3, 3); ctx.fillEllipse(25, 15, 3, 3);
      // Fire hooves
      ctx.fillStyle = '#ff6d00';
      ctx.fillEllipse(12, 50, 10, 8); ctx.fillEllipse(28, 50, 10, 8);
      ctx.fillEllipse(40, 50, 10, 8); ctx.fillEllipse(56, 50, 10, 8);
    });

    // === ZONE 6 ===
    // Archangel (52x64)
    TextureGenerator.makeSimpleMonster(scene, 'Archangel', 52, 64, (ctx) => {
      ctx.fillStyle = '#fff9c4'; ctx.fillEllipse(26, 38, 40, 46); // golden robe body
      ctx.fillStyle = '#ffe082'; ctx.fillEllipse(26, 18, 22, 24); // head
      // halo
      ctx.strokeStyle = '#ffd700'; ctx.lineWidth = 4;
      ctx.beginPath(); ctx.arc(26, 8, 12, 0, Math.PI * 2); ctx.stroke();
      // wings
      ctx.fillStyle = 'rgba(255,255,255,0.9)';
      ctx.beginPath(); ctx.moveTo(26, 22); ctx.lineTo(2, 6); ctx.lineTo(4, 36); ctx.lineTo(26, 32); ctx.fill();
      ctx.beginPath(); ctx.moveTo(26, 22); ctx.lineTo(50, 6); ctx.lineTo(48, 36); ctx.lineTo(26, 32); ctx.fill();
      ctx.fillStyle = '#64b5f6'; ctx.fillEllipse(21, 18, 6, 6); ctx.fillEllipse(31, 18, 6, 6); // blue eyes
      ctx.fillStyle = '#fff'; ctx.fillEllipse(20, 17, 2.5, 2.5); ctx.fillEllipse(30, 17, 2.5, 2.5);
      // Holy sword
      ctx.fillStyle = '#ffd700'; ctx.fillRect(24, 36, 4, 28); ctx.fillRect(16, 44, 20, 4);
    });
    // LordBaphomet (68x72)
    TextureGenerator.makeSimpleMonster(scene, 'LordBaphomet', 68, 72, (ctx) => {
      ctx.fillStyle = '#1a0030'; ctx.fillEllipse(34, 46, 56, 52); // dark body
      ctx.fillStyle = '#2d0050'; ctx.fillEllipse(34, 22, 30, 30); // head
      // massive curved horns
      ctx.strokeStyle = '#4a0080'; ctx.lineWidth = 6;
      ctx.beginPath(); ctx.moveTo(22, 12); ctx.quadraticCurveTo(4, -2, 8, 14); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(46, 12); ctx.quadraticCurveTo(64, -2, 60, 14); ctx.stroke();
      // bat wings
      ctx.fillStyle = '#1a0030';
      ctx.beginPath(); ctx.moveTo(34, 28); ctx.lineTo(2, 16); ctx.lineTo(6, 50); ctx.lineTo(34, 44); ctx.fill();
      ctx.beginPath(); ctx.moveTo(34, 28); ctx.lineTo(66, 16); ctx.lineTo(62, 50); ctx.lineTo(34, 44); ctx.fill();
      ctx.fillStyle = '#ff1744'; ctx.fillEllipse(27, 22, 8, 9); ctx.fillEllipse(41, 22, 8, 9); // red eyes
      ctx.fillStyle = '#fff'; ctx.fillEllipse(26, 21, 3, 3.5); ctx.fillEllipse(40, 21, 3, 3.5);
      // pentagram on chest
      ctx.strokeStyle = '#ff1744'; ctx.lineWidth = 1.5;
      ctx.beginPath(); ctx.moveTo(34, 36); ctx.lineTo(38, 44); ctx.lineTo(28, 38); ctx.lineTo(40, 38); ctx.lineTo(30, 44); ctx.closePath(); ctx.stroke();
      // scythe
      ctx.fillStyle = '#546e7a'; ctx.fillRect(60, 20, 5, 48); // handle
      ctx.fillStyle = '#b0bec5'; // blade
      ctx.beginPath(); ctx.moveTo(65, 20); ctx.quadraticCurveTo(80, 28, 62, 38); ctx.lineTo(65, 38); ctx.closePath(); ctx.fill();
    });
    // Valkyrie (56x64)
    TextureGenerator.makeSimpleMonster(scene, 'Valkyrie', 56, 64, (ctx) => {
      ctx.fillStyle = '#e3f2fd'; ctx.fillEllipse(28, 42, 44, 46); // silver armor body
      ctx.fillStyle = '#90caf9'; ctx.fillEllipse(28, 20, 24, 28); // head
      // winged helm
      ctx.fillStyle = '#b0bec5'; ctx.fillRect(16, 10, 24, 12);
      ctx.fillStyle = '#b0bec5';
      ctx.fillTriangle(16, 10, 6, 2, 14, 14); ctx.fillTriangle(40, 10, 50, 2, 42, 14);
      // gold trim
      ctx.strokeStyle = '#ffd700'; ctx.lineWidth = 2.5;
      ctx.strokeRect(16, 10, 24, 12);
      ctx.fillStyle = '#1565c0'; ctx.fillEllipse(22, 20, 6, 6); ctx.fillEllipse(34, 20, 6, 6); // blue eyes
      ctx.fillStyle = '#fff'; ctx.fillEllipse(21, 19, 2.5, 2.5); ctx.fillEllipse(33, 19, 2.5, 2.5);
      // divine wings
      ctx.fillStyle = 'rgba(255,253,231,0.85)';
      ctx.beginPath(); ctx.moveTo(28, 26); ctx.lineTo(2, 12); ctx.lineTo(4, 42); ctx.lineTo(28, 38); ctx.fill();
      ctx.beginPath(); ctx.moveTo(28, 26); ctx.lineTo(54, 12); ctx.lineTo(52, 42); ctx.lineTo(28, 38); ctx.fill();
      // spear
      ctx.fillStyle = '#b0bec5'; ctx.fillRect(50, 16, 4, 44); // shaft
      ctx.fillStyle = '#ffd700'; ctx.fillTriangle(52, 14, 46, 26, 58, 26); // tip
    });
  }

  /** Helper: create a canvas texture using a drawing function */
  private static makeSimpleMonster(scene: Phaser.Scene, key: string, w: number, h: number, draw: (ctx: ExtCtx) => void) {
    if (scene.textures.exists(key)) return;
    const canvas = scene.textures.createCanvas(key, w, h);
    if (!canvas) return;
    const ctx = canvas.getContext() as ExtCtx;
    // Polyfills for convenience
    ctx.fillEllipse = (x: number, y: number, ew: number, eh: number) => {
      ctx.save(); ctx.translate(x, y); ctx.scale(ew / 2, eh / 2);
      ctx.beginPath(); ctx.arc(0, 0, 1, 0, Math.PI * 2); ctx.restore(); ctx.fill();
    };
    ctx.fillTriangle = (x1: number, y1: number, x2: number, y2: number, x3: number, y3: number) => {

...
import { CharacterModel, InventoryItem } from './types.js';
...
export class PostgresTransactionManager {
   ...
   */
  private async acquireRowLocks(rowIds: string[]): Promise<() => void> {
   ...
   */
  public async executeRefineTransaction(characterId: string, itemId: string): Promise<RefineResult> {
   ...
   */
  private async executePostgresRefine(characterId: string, itemId: string): Promise<RefineResult> {
   ...
   */
  private async executeEmulatedRefine(characterId: string, itemId: string): Promise<RefineResult> {
    ...
    try {
      ...
      let targetItem: InventoryItem | undefined = char.equipped.weapon?.id === itemId ? char.equipped.weapon : undefined;
      ...
      const currentRefine = (targetItem as any).refine || 0;
      ...
      if (isSuccess) {
        const nextRefine = currentRefine + 1;
        (targetItem as any).refine = nextRefine;
        if (!targetItem.effect) targetItem.effect = {};

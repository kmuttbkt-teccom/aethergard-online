...
export interface BotSettings {
  ...
  autoPotion: boolean;
  autoLoot: boolean;
  hpThreshold: number; // percentage (e.g. 45)
  mpThreshold: number; // percentage (e.g. 30)
}
...
export class BotEngine {
  ...
  private lastPotionTime = 0;
  private lastLootTime = 0;
  private lastAttackTime = 0;
  private onStateChange?: (running: boolean) => void;

  constructor(onChange?: (running: boolean) => void) {
    this.onStateChange = onChange;
  }

  public setOnChange(cb: (running: boolean) => void) {
  ...
  public update(scene: WorldScene, dt: number): { moveLeft: boolean; moveRight: boolean; jump: boolean } {
    const inputOverride = { moveLeft: false, moveRight: false, jump: false };
    if (!this.enabled || !scene.selfData) return inputOverride;
        ...
        if (dist <= 85) {
          if (now - this.lastAttackTime > 450) {
            ...
            if (this.settings.autoSkill && player.mp >= 20) {
              if (player.job === 'Swordman') skillToUse = Math.random() > 0.4 ? 'RADIANT_SLASH' : 'BASH';
              else if (player.job === 'Magician') skillToUse = Math.random() > 0.4 ? 'ASTRAL_METEOR' : 'FROST_NOVA';
              else if (player.job === 'Archer') skillToUse = 'GALE_ARROW';
              else if (player.job === 'Thief') skillToUse = Math.random() > 0.5 ? 'SHADOW_BLINK' : 'BLADE_DANCE';
              else if (player.job === 'Acolyte') skillToUse = 'SANCTUARY';
              else if (player.job === 'Merchant') skillToUse = player.zeny > 100 ? 'COIN_BURST' : 'NORMAL';
            }

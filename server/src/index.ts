...
function authResponse(result: { account?: any; token?: string; guestSecret?: string }) {
...
function getAuthAccount(req: IncomingMessage) {
...
const server = createServer(async (req, res) => {
  ...
  if (pathname === '/api/auth/guest' && req.method === 'POST') {
    ...
    const result = db.loginGuest(
      typeof body.guestId === 'string' ? body.guestId : undefined,
      typeof body.guestSecret === 'string' ? body.guestSecret : undefined
    );

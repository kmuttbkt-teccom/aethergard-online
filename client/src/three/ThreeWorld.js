...
import { SKILL_DB, JOB_SKILL_BAR, SERVER_NPC_ACTIONS, canUseSkill, computeDerived, inferWeaponType, isBossMonster, toWorld3DX, toWorld3DZ, fromWorld3DX, fromWorld3DZ, WORLD_MAX_X, WORLD_MAX_Z, WORLD_SCALE, RARITY_COLORS } from '../../../server/src/GameData.js';
import { CharacterModelBuilder } from './CharacterModelBuilder.js';
import { MonsterModelBuilder } from './MonsterModelBuilder.js';
import { ThreeTerrain } from './ThreeTerrain.js';
import { sound } from '../engine/Sound.js';
import { NPC3DManager } from './NPC3DManager.js';
...
class OverheadLabel {
    ...
    update(name, level, hp, maxHp, highlight = false) {
...
export class ThreeWorld {
    ...
    playerTarget = null;
    playerFacing = 0; // Y angle in radians
    isMoving = false;
    isJumping = false;
    jumpVy = 0;
    ...
    cameraDistance = 14;
    cameraPitch = Math.PI * 0.28; // ~50 degrees elevation
    cameraYaw = 0; // 360 degree rotation
    ...
    targetReticle;
    footstepTimer = 0;
    footstepDustGroup = new THREE.Group();
    // Animation Loop
    animFrameId = 0;
    lastTime = 0;
    keys = {};
    ...
    initThree() {
        ...
        this.playerVisuals = CharacterModelBuilder.createCharacter(this.character.name, this.character.job, this.character.gender, this.character.hairColor || '#ffd700', Boolean(this.character.isGm));
        ...
        this.dungeonManager.setCallback((state) => {
            this.ui.updateBossRaidBar(state.isActive, state.stageName, state.bossHp, state.bossMaxHp, state.bossType);
        });
    }
    start() {
        this.isRunning = true;
    ...
    placeAt(serverX, serverZ) {
        this.playerPos.set(Math.max(MIN_X3D, Math.min(MAX_X3D, toWorld3DX(serverX))), 0, Math.max(-MAX_Z3D, Math.min(MAX_Z3D, toWorld3DZ(serverZ))));
    ...
    onKeyDown = (e) => {
        if (!this.isRunning || this.isTyping())
            return;
    ...
    handleGroundClick(e) {
        ...
        if (intersects.length > 0) {
            ...
            this.clickMarker.material.opacity = 0.9;
        }
    }
    // ===========================================================================
    // Targeting
    ...
    performAttack(skillName = 'NORMAL') {
    ...
    performSkill(skillId) {
        ...
        if (skill.kind === 'basic') {
            if (now - this.lastBasicAt < computeDerived(self).aspdMs)
                return;
        }
        ...
        if (needsTarget) {
            ...
            if (!target) {
                ...
                if (!this.autoAttack)
                    this.ui.gameplay.toast('ไม่มีมอนสเตอร์อยู่ใกล้ ๆ', 'error');
                return;
            }
            if (target.data.id !== this.selectedTargetId)
                this.selectTarget(target.data.id);
    ...
    usePotion(kind, silent = false) {
        if (!this.selfData || this.isDead)
            return false;
        ...
        if (!potion) {
            if (!silent)
                this.ui.gameplay.toast(kind === 'hp' ? '🧪 ไม่มียาฟื้นฟู HP' : '💙 ไม่มียาฟื้นฟู MP', 'error');
            return false;
        }
        sound.playPotion();
    ...
    updateBot() {
        ...
        if (!bot?.enabled || !this.selfData)
            return;
        const now = performance.now();
        if (now < this.botNextThinkAt)
            return;
        ...
        if (this.isDead) {
            if (!this.botDeadSince)
                this.botDeadSince = now;
        ...
        if (s.autoPotion && now - this.botLastPotionAt > 1100) {
            if ((self.hp / self.maxHp) * 100 <= s.hpThreshold && this.usePotion('hp', true))
                this.botLastPotionAt = now;
            else if ((self.mp / self.maxMp) * 100 <= s.mpThreshold && this.usePotion('mp', true))
                this.botLastPotionAt = now;
        }
        // 2. Self-heal skills when hurt
        const bar = JOB_SKILL_BAR[self.job];
        const jobSkills = [bar[0], bar[1]].filter((id) => Boolean(id)).map(id => SKILL_DB[id]);
        ...
        if (target) {
            ...
            if (s.autoSkill && !this.queuedSkill) {
                const offensive = [...jobSkills, SKILL_DB.BASH].filter(sk => sk && sk.kind !== 'heal' && sk.kind !== 'utility' && canUseSkill(sk, self.job)
        ...
        if (far && !this.playerTarget)
            this.playerTarget = far.pos.clone();
    }
    /** Walk to the nearest drop within reach; returns true while looting */
    botLoot() {
        if (this.pendingPickupId && this.drops.has(this.pendingPickupId))
            return true;
        let best;
        let bestDist = 12;
    ...
    enterDungeon(floor = 1) {
        ...
        this.scene.background.setHex(0x050a18);
    ...
    exitDungeon() {
        ...
        this.scene.background.setHex(0xa3cef1);
        this.scene.fog = new THREE.FogExp2(0xa3cef1, 0.015);
    ...
    toggleAfkRaid() {
        ...
        if (label)
            label.textContent = `⚔️ AFK Raid: ${this.isAfkRaid ? 'ON ✅' : 'OFF'}`;
        if (this.isAfkRaid) {
    ...
    updateAfkRaid(time) {
        if (!this.isAfkRaid || !this.dungeonManager?.state?.isActive)
            return;
        if (time < this.afkRaidNextAt)
            return;
        ...
        if (this.dungeonManager.state.bossHp > 0) {
            ...
            this.dungeonManager.updateBossHp(Math.max(0, this.dungeonManager.state.bossHp - dmg), this.dungeonManager.state.bossMaxHp);
        }
    }
    subscribeNetwork() {
        const on = (type, fn) => this.unsubscribers.push(this.network.on(type, fn));
        on('INIT_STATE', (msg) => {
            this.selfId = msg.selfId;
            this.selfData = msg.player;
            this.isDead = Boolean(msg.player?.isDead);
            (msg.drops || []).forEach(d => this.addDrop(d));
        });
        on('PLAYER_ATTACKED', (msg) => this.onPlayerAttacked(msg));
        on('DAMAGE', (msg) => this.onDamage(msg.damageEvent, msg.mobHp, msg.delayMs || 0));
        on('MOB_DIED', (msg) => {
            ...
            if (msg.mobId === this.selectedTargetId)
                this.clearTarget();
            (msg.drops || []).forEach(d => this.addDrop(d));
        ...
        on('SKILL_USED', (msg) => {
            ...
            if (skill && skill.kind !== 'basic')
                this.ui.gameplay.startCooldown(msg.skillId, msg.cooldownMs);
        });
        ...
        on('PLAYER_HEALED', (msg) => {
            ...
            if (pos)
                this.effects.floatingText(new THREE.Vector3(pos.x, 2.6, pos.z), `+${msg.amount}`, '#86efac', '#14532d');
    ...
    onPlayerAttacked(msg) {
        ...
        let weaponType = 'fist';
        if (isSelf)
            weaponType = inferWeaponType(this.selfData?.equipped.weapon);
        else
            weaponType = remote.data.weaponType || 'fist';
        if (!isSelf) {
            remote.visuals.playAttack(this.elapsed);
            if (msg.rotY !== undefined)
                remote.targetRot = msg.rotY;
        }
        // Skip effects far off-screen
        if (casterPos.distanceTo(this.playerPos) > 60)
            return;
        this.effects.playSkill(msg.skill, casterPos, rotY, targetPos, center, weaponType);
        if (isSelf || casterPos.distanceTo(this.playerPos) < 25)
            sound.playSlash();
    }
    onDamage(event, mobHp, delayMs) {
        ...
        if (mobHp !== undefined) {
            ...
            if (mob.data.id === this.selectedTargetId)
                this.refreshTargetFrame(mob);
        }
        const show = () => {
            if (mob.pos.distanceTo(this.playerPos) > 60)
                return;
            mob.hitT = 1;
    ...
    onPlayerHit(msg) {
        ...
        if (isSelf && this.selfData) {
            ...
            if (!this.selectedTargetId && mob)
                this.selectTarget(mob.data.id);
        }
        if (!this.isRunning)
            return;
        ...
        if (msg.isMiss) {
            ...
            if (isSelf) {
                ...
                if (this.hitVignette) {
                    ...
                    setTimeout(() => { if (this.hitVignette)
                        this.hitVignette.style.opacity = '0'; }, 160);
                }
                sound.playHit(false);
            }
        }
    ...
    syncSnapshot(snapshot) {
        ...
        snapshot.monsters.forEach(m => {
            ...
            if (m.id === this.selectedTargetId)
                this.refreshTargetFrame(mob);
        });
    }
    createMonsterEntry(m, targetPos) {
    ...
    addDrop(d) {
        ...
        const ring = new THREE.Mesh(new THREE.RingGeometry(0.25, 0.4, 20).rotateX(-Math.PI / 2), new THREE.MeshBasicMaterial({ color: beamColor, transparent: true, opacity: 0.6, side: THREE.DoubleSide, depthWrite: false }));
        ring.position.y = 0.04;
        group.add(ring);
        if (d.type === 'equip' || d.type === 'card') {
            const beam = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.2, 3, 8, 1, true), new THREE.MeshBasicMaterial({ color: beamColor, transparent: true, opacity: 0.3, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide }));
            beam.position.y = 1.5;
            group.add(beam);
        }
    ...
    renderLoop = (timestamp) => {
        ...
        const animState = this.isDead ? 'dead' : this.isJumping ? 'jump' : (this.isMoving ? 'walk' : 'idle');
        ...
        this.remotePlayers.forEach(r => {
            ...
            const anim = r.data.isDead ? 'dead' : r.data.anim === 'jump' ? 'jump' : moving || r.data.anim === 'walk' ? 'walk' : 'idle';
    ...
    updateCombatIntent() {
        if (this.isDead || (!this.autoAttack && !this.queuedSkill))
            return;
        const target = this.selectedTargetId ? this.monsters.get(this.selectedTargetId) : undefined;
    ...
    syncPosition(force = false) {
        const now = performance.now();
        if (!force && now - this.lastPosSyncAt < 100)
            return;
    ...
    spawnFootstepDust(pos) {
        ...
        dust.position.set(pos.x + (Math.random() - 0.5) * 0.3, 0.08, pos.z + (Math.random() - 0.5) * 0.3);
    ...
    handleNpcAction(npc, option) {
        ...
        if (SERVER_NPC_ACTIONS.includes(option.action)) {
            ...
            if (option.action === 'HEAL_FULL' || option.action === 'BLESSING')
                sound.playBlessing();
            return;
        }
    ...
    async setPlayerCustomModel(urlOrId) {
        try {
    ...
    setPlayerWeaponSkin(weaponId) {
        ...
        switch (weaponId) {
            case 'weapon_excalibur':
                visual = Weapon3DBuilder.createExcalibur();
                break;
            case 'weapon_dragonslayer':
                visual = Weapon3DBuilder.createDragonSlayer();
                break;
            case 'weapon_staff':
                visual = Weapon3DBuilder.createArchangelStaff();
                break;
            case 'weapon_bow':
                visual = Weapon3DBuilder.createAstralBow();
                break;
            case 'weapon_dagger':
                visual = Weapon3DBuilder.createNightshadeDagger();
                break;
            case 'weapon_shield':

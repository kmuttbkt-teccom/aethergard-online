import { MonsterData, MonsterType, DropItemData, ZoneDef, ZoneId, PlayerData } from './types.js';
import { getMonsterAiProfile, isBossMonster, WORLD_MAX_Z } from './GameData.js';

/** A monster landed a swing on a player this tick — Room resolves the damage */
 ...
 */
export function combatDistance(player: { x: number; y: number; z?: number; viewMode?: '2d' | '3d' }, target: { x: number; y: number; z?: number }): number {
...
export const ZONES_CONFIG: ZoneDef[] = [
  {
    id: 'solaria_meadows',
...
export class MonsterManager {
  ...
  public drops: Map<string, DropItemData> = new Map();
  private nextMobId = 1;
  ...
  private initMonsters() {
    const spawns: Array<{ type: MonsterType; name: string; x: number; y: number; minX: number; maxX: number }> = [
  ...
  public spawnMonster(type: MonsterType, name: string, x: number, y: number, minX: number, maxX: number) {
    ...
    switch (type) {
      // Zone 1
      case 'Sunbun':
        hp = 70; level = 2; exp = 22; jobExp = 18; atk = 9; def = 1; break;
      case 'Poring':
        hp = 85; level = 3; exp = 28; jobExp = 22; atk = 11; def = 2; break;
      case 'Leafkin':
        hp = 130; level = 6; exp = 55; jobExp = 45; atk = 16; def = 4; break;
      case 'Fabre':
        hp = 160; level = 8; exp = 75; jobExp = 60; atk = 22; def = 5; break;
      case 'Spore':
        hp = 240; level = 12; exp = 125; jobExp = 100; atk = 32; def = 8; break;

      // Zone 2
      case 'Aethercap':
        hp = 420; level = 18; exp = 220; jobExp = 180; atk = 48; def = 12; break;
      case 'Gryphlet':
        hp = 650; level = 24; exp = 380; jobExp = 310; atk = 68; def = 18; break;
      case 'PecoPeco':
        hp = 820; level = 27; exp = 490; jobExp = 400; atk = 80; def = 22; break;
      case 'ForestBoar':
        hp = 1050; level = 30; exp = 680; jobExp = 550; atk = 96; def = 28; break;
      case 'WildWolf':
        hp = 1300; level = 34; exp = 890; jobExp = 720; atk = 118; def = 32; break;

      // Zone 3
      case 'DesertScorpion':
        hp = 1750; level = 38; exp = 1250; jobExp = 1000; atk = 142; def = 40; break;
      case 'SandGolem':
        hp = 2600; level = 45; exp = 1950; jobExp = 1600; atk = 175; def = 68; break;
      case 'MummyPharaoh':
        hp = 3400; level = 52; exp = 2800; jobExp = 2300; atk = 210; def = 52; break;
      case 'RedDemon':
        hp = 4200; level = 55; exp = 3600; jobExp = 3000; atk = 245; def = 60; break;

      // Zone 4
      case 'SkeletonSoldier':
        hp = 5200; level = 60; exp = 4600; jobExp = 3900; atk = 290; def = 75; break;
      case 'WraithPhantom':
        hp = 6400; level = 66; exp = 5800; jobExp = 4900; atk = 340; def = 85; break;
      case 'DarkGargoyle':
        hp = 7800; level = 72; exp = 7200; jobExp = 6100; atk = 395; def = 95; break;

      // Zone 5
      case 'FireSalamander':
        hp = 9500; level = 78; exp = 9200; jobExp = 7800; atk = 460; def = 110; break;
      case 'LavaGolem':
        hp = 12500; level = 84; exp = 12500; jobExp = 10500; atk = 540; def = 145; break;
      case 'NightmareSteed':
        hp = 15500; level = 88; exp = 16000; jobExp = 13500; atk = 620; def = 130; break;

      // Zone 6 & MVPs
      case 'Archangel':
        hp = 21000; level = 92; exp = 24000; jobExp = 20000; atk = 750; def = 160; break;
      case 'BaphometJr':
        hp = 32000; level = 95; exp = 38000; jobExp = 32000; atk = 920; def = 190; break;
      case 'Solarion':
        hp = 85000; level = 99; exp = 120000; jobExp = 100000; atk = 1450; def = 260; break;
      case 'LordBaphomet':
   ...
   */
  public update(dt: number, players: PlayerData[], now: number = Date.now()): MonsterAttackEvent[] {
    const attacks: MonsterAttackEvent[] = [];
    const alivePlayers = players.filter(p => !p.isDead && p.hp > 0);

    this.monsters.forEach((mob) => {
      if (mob.isDead) return;
      if ((mob.frozenUntil || 0) > now) return;
      ...
      if (target) {
        ...
        if (dist > range * 0.9) {
          ...
          mob.x += (dx / len) * step;
          if (use3D) mob.z = (mob.z || 0) + (dz / len) * step;
  ...
  public handleMobDeath(mob: MonsterData): DropItemData[] {
    mob.isDead = true;
    this.dropZ = mob.z || 0;
    const drops: DropItemData[] = [];
  ...
  public createDrop(x: number, y: number, type: DropItemData['type'], name: string, icon: string, amount: number, itemData?: any): DropItemData {
    const id = `drop_${this.nextDropId++}`;
    const drop: DropItemData = {
      id,
      name,
      type,
      icon,

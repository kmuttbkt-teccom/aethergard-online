...
import { Account, CharacterModel, Stats, InventoryItem } from './types.js';
...
export class Database {
  ...
  private loadTokenSecret(dataDir: string): string {
  ...
  private verifyPassword(account: Account, password: string): boolean {
    ...
    if (!safeEqual(legacy, stored)) return false;
   ...
   */
  public loginGuest(existingGuestId?: string, guestSecret?: string): { account: Account; token: string; guestSecret?: string } {
  ...
  public registerLocal(username: string, password: string, displayName?: string): { account?: Account; token?: string; error?: string } {
  ...
  public loginLocal(username: string, password: string): { account?: Account; token?: string; error?: string } {
    const cleanUser = String(username).trim().toLowerCase();
  ...
  public loginOAuth(provider: 'google' | 'facebook', profile: { email: string; name: string; id: string }): { account: Account; token: string } {
  ...
  // --- Character Methods ---
  public getCharactersByAccount(accountId: string): CharacterModel[] {
  ...
  public createCharacter(
    ...
    hairStyle: number,
    hairColor: string
  ): { character?: CharacterModel; error?: string } {
    ...
    const now = Date.now();
    const initialWeapon: InventoryItem = makeItem('novice_knife')!;

    const gmWeapon: InventoryItem = {
  ...
  public deleteCharacter(accountId: string, characterId: string): boolean {
  ...
  public getCharacterById(characterId: string): CharacterModel | null {
    return this.data.characters.find(c => c.id === characterId) || null;
  }

  public saveCharacter(char: CharacterModel) {
  ...
  public setAccountRole(accountId: string, role: 'user' | 'gm' | 'admin'): boolean {

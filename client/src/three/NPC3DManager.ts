...
import { ModelAssetManager, Loaded3DAsset } from './ModelAssetManager.js';
import { sound } from '../engine/Sound.js';
import { NPC_DEFS, NpcDef, NpcOption } from '../../../server/src/GameData.js';

export type NPCDef = NpcDef;
...
export class NPC3DManager {
  private static _instance: NPC3DManager | null = null;
  public scene: THREE.Scene | null = null;
  public npcs: Map<string, NPCInstance> = new Map();
  public onNpcInteractCallback?: (npc: NPCDef, option: NpcOption) => void;

  public static getInstance(): NPC3DManager {
    if (!this._instance) {
  ...
  public async initNpcs(scene: THREE.Scene) {
    this.scene = scene;
    this.npcs.clear();
  ...
  private _createNameTag(name: string, role: string, badge: string): THREE.Sprite {
  ...
  private _buildProceduralNpc(group: THREE.Group, def: NPCDef) {
   ...
   */
  public checkRaycast(raycaster: THREE.Raycaster): NPCDef | null {
    const hitboxMeshes: THREE.Object3D[] = [];
    this.npcs.forEach(({ group }) => {
      group.traverse((child) => {
        if (child.userData?.isNpc) hitboxMeshes.push(child);
      });
    });
   ...
   */
  public openDialogue(npc: NPCDef, onAction: (option: NpcOption) => void) {
    sound.playNpc();
    ...
    modal.querySelectorAll('.npc-opt-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        const option = npc.options[Number((e.currentTarget as HTMLElement).dataset.index)];
        if (modal) modal.style.display = 'none';
        if (option) onAction(option);
      });
    });
  }
}

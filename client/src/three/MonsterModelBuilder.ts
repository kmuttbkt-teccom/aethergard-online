...
import { ModelLoader, LoadedModel } from './ModelLoader.js';

export interface Monster3DVisuals {
  group: THREE.Group;
  type: MonsterType;
  updateAnimation: (time: number, dt: number) => void;
  /** The GLTF LoadedModel if loaded from file, null if procedural */
 ...
 */
const MONSTER_GLB_MAP: Partial<Record<MonsterType, string>> = {
  // Zone 1
  'Poring':          'mob_duck.glb',
  'Sunbun':          'mob_duck.glb',
...
export class MonsterModelBuilder {
   ...
   */
  public static async createMonsterAsync(
    type: MonsterType,
    name: string,
    scene: THREE.Scene
  ): Promise<Monster3DVisuals> {
      ...
      try {
        ...
        if (model) {
          ...
          return {
            ...
            type,
            gltfModel: model,
            updateAnimation: (_time: number, dt: number) => {
  ...
  private static _addNameTag(group: THREE.Group, name: string, _type: MonsterType, posY = 2.8) {
  ...
  public static createMonster(type: MonsterType, name: string): Monster3DVisuals {
    const group = new THREE.Group();
    group.name = `mob_${type}_${name}`;

    let updateAnimation: (time: number, dt: number) => void = () => {};
    ...
    switch (type) {
      ...
      case 'WildWolf': {
        ...
        body.scale.set(1.3, 0.8, 1.0); body.position.y = 0.38; body.castShadow = true; group.add(body);
        const head = new THREE.Mesh(new THREE.SphereGeometry(0.28, 10, 10), bodyMat);
        head.position.set(0.42, 0.42, 0); group.add(head);
      ...
      case 'MummyPharaoh': {
        ...
        if (type === 'MummyPharaoh') {
          ...
          crown.position.y = 1.64; group.add(crown);
        }
        ...
        [-0.14, 0.14].forEach(ex => {
          ...
          eye.position.set(ex, 1.36, 0.24); group.add(eye);
        });
        // Arms
        [-1, 1].forEach(side => {
          const arm = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.6, 0.22), mat);
          arm.position.set(side * 0.46, 0.7, 0); group.add(arm);
        });
      ...
      case 'SkeletonSoldier': {
        ...
        sword.position.set(0.42, 0.85, 0); group.add(sword);
      ...
      case 'DarkGargoyle': {
        ...
        body.scale.set(0.9, 1.4, 0.9); body.position.y = 0.7; group.add(body);
        ...
        if (type === 'DarkGargoyle') {
          ...
          [-1, 1].forEach(side => {
            ...
            wing.position.set(side * 0.6, 1.1, -0.1); wing.rotation.y = side * 0.4; group.add(wing);
          });
        }
        const eyeMat = new THREE.MeshBasicMaterial({ color: 0xff1744 });
      ...
      case 'FireSalamander': {
        ...
        body.scale.set(1.4, 0.8, 1.0); body.position.y = 0.36; body.castShadow = true; group.add(body);
        const head = new THREE.Mesh(new THREE.SphereGeometry(0.26, 10, 10), fireMat);
        head.position.set(0.56, 0.44, 0); group.add(head);
        const tail = new THREE.Mesh(new THREE.ConeGeometry(0.06, 0.5, 8), new THREE.MeshStandardMaterial({ color: 0xff7043 }));
        tail.position.set(-0.62, 0.4, 0); tail.rotation.z = Math.PI * 0.5; group.add(tail);
      ...
      case 'LavaGolem': {
        ...
        crack1.position.set(0.12, 0.8, 0.36); group.add(crack1);
        ...
        [-1, 1].forEach(side => {
          ...
          fist.position.set(side * 0.78, 0.64, 0); group.add(fist);
        });
      ...
      case 'NightmareSteed': {
        ...
        neck.position.set(0.34, 0.9, 0); neck.rotation.z = 0.35; group.add(neck);
        const head = new THREE.Mesh(new THREE.BoxGeometry(0.36, 0.28, 0.28), darkMat);
        head.position.set(0.62, 1.06, 0); group.add(head);
        ...
        [-0.3, 0.3].forEach(fx => {
          ...
          hoof.position.set(fx, 0.1, 0.2); group.add(hoof);
          const hoof2 = new THREE.Mesh(new THREE.SphereGeometry(0.09), hoofMat);
          hoof2.position.set(fx, 0.1, -0.2); group.add(hoof2);
        });
      ...
      case 'Archangel': {
        ...
        head.position.y = 1.34; group.add(head);
        // Halo
        ...
        halo.position.y = 1.68; group.add(halo);
        // Wings
        ...
        [-1, 1].forEach(side => {
          ...
          wing.position.set(side * 0.56, 1.08, -0.12); wing.rotation.y = side * 0.35; group.add(wing);
        });
        ...
        blade.position.set(0.46, 0.7, 0); group.add(blade);
        const guard = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.06, 0.04), swordMat);
        guard.position.set(0.46, 0.9, 0); group.add(guard);
      ...
      case 'LordBaphomet': {
        ...
        [-1, 1].forEach(side => {
          ...
          horn.position.set(side * 0.26, 1.86, 0); horn.rotation.z = side * 0.6; group.add(horn);
        });
        // Bat wings
        ...
        [-1, 1].forEach(side => {
          ...
          wing.position.set(side * 0.88, 1.2, -0.15); wing.rotation.y = side * 0.4; group.add(wing);
        });
        const eyeMat = new THREE.MeshBasicMaterial({ color: 0xff1744 });
        ...
        handle.position.y = 0.5; scythe.add(handle);
        const blade = new THREE.Mesh(new THREE.ConeGeometry(0.2, 0.45, 3), new THREE.MeshStandardMaterial({ color: 0xb0bec5, metalness: 0.8 }));
        blade.position.set(0.18, 0.98, 0); blade.rotation.z = -0.4; scythe.add(blade);
        scythe.position.set(0.72, 0.7, 0); group.add(scythe);
        // MVP boss scale
      ...
      case 'Valkyrie': {
        ...
        head.position.y = 1.36; group.add(head);
        // Winged helm
        ...
        helm.position.y = 1.56; group.add(helm);
        [-1, 1].forEach(side => {
          const hw = new THREE.Mesh(new THREE.PlaneGeometry(0.18, 0.22), helmMat);
          hw.position.set(side * 0.28, 1.62, 0); hw.rotation.y = side * 0.45; group.add(hw);
        });
        // Divine wings
        ...
        [-1, 1].forEach(side => {
          ...
          wing.position.set(side * 0.56, 1.06, -0.1); wing.rotation.y = side * 0.35; group.add(wing);
        });
        ...
        spear.position.set(0.52, 0.84, 0); group.add(spear);
        ...
        speartip.position.set(0.52, 1.52, 0); group.add(speartip);

...
import { JobClass, InventoryItem, ItemRarity, WeaponType } from '../../../server/src/types.js';
...
export type CharacterAnim = 'idle' | 'walk' | 'jump' | 'attack' | 'dead';

export interface Character3DVisuals {
  group: THREE.Group;
  bodyMesh: THREE.Mesh;
  headMesh: THREE.Mesh;
  hairMesh: THREE.Mesh;
  ...
  backSlot: THREE.Group;
  crownMesh?: THREE.Group;
  wingsMesh?: THREE.Group;
  auraMesh?: THREE.Mesh;
  capeMesh?: THREE.Group;
  updateAnimation: (anim: CharacterAnim, time: number, dt: number) => void;
  setEquipment: (equipped: { weapon?: InventoryItem; head?: InventoryItem; accessory?: InventoryItem }) => void;
  /** Swap the hand-held weapon; no-op when the same weapon is already shown */
  setWeapon: (weaponType: WeaponType | undefined, refine?: number, rarity?: ItemRarity, isExcalibur?: boolean) => void;
  /** Start a one-shot attack swing (plays for ~0.35s) */
...
export class CharacterModelBuilder {
  ...
  public static createCharacter(
    ...
    if (isGm) bodyColor = 0xffd700; // Gold for GM
    ...
    else if (job === 'Acolyte') bodyColor = 0xf5f3f4;
    ...
    let crownMesh: THREE.Group | undefined;
    ...
    const playAttack = (time: number) => {
      attackStartedAt = time;
    };

    const updateAnimation = (animIn: CharacterAnim, time: number, dt: number) => {
      // A queued swing overrides idle/walk until it finishes
      let anim: CharacterAnim = animIn;
      const swingT = (time - attackStartedAt) / ATTACK_DURATION;
      if (anim !== 'dead' && swingT >= 0 && swingT < 1) anim = 'attack';
    ...
    const setWeapon = (weaponType: WeaponType | undefined, refine = 0, rarity: ItemRarity = 'common', isExcalibur = false) => {
      ...
      weaponSlot.clear();
      shieldSlot.clear();
      if (!weaponType || weaponType === 'fist') return;
    ...
    const setEquipment = (equipped: { weapon?: InventoryItem; head?: InventoryItem; accessory?: InventoryItem }) => {
      ...
      setWeapon(
        inferWeaponType(w),
        w.refine || 0,
        w.rarity || (w.id.startsWith('gm_') ? 'legendary' : 'common'),
        w.id.includes('excalibur')
      );
    };
   ...
   */
  private static applyClassAppearance(
    job: JobClass,
    headSlot: THREE.Group,
    weaponSlot: THREE.Group,
    shieldSlot: THREE.Group,
   ...
   */
  public static createExcaliburSword(): THREE.Group {
  ...
  public static createSteelSword(): THREE.Group {
    const sword = new THREE.Group();
  ...
  public static createKiteShield(): THREE.Group {
  ...
  public static createMagicStaff(): THREE.Group {
  ...
  public static createHolyStaff(): THREE.Group {
    const staff = new THREE.Group();
  ...
  public static createWoodenBow(): THREE.Group {
  ...
  public static createDagger(): THREE.Group {
  ...
  public static createClub(): THREE.Group {
   ...
   */
  public static createGmCrown(): THREE.Group {
   ...
   * 3D Seraph Wings [GM]
   */
  public static createSeraphWings(): THREE.Group {
   ...
   */
  public static async tryLoadGLTFPlayer(job: JobClass): Promise<THREE.Group | null> {
    // Map jobs to character models
    const glbMap: Partial<Record<JobClass, string>> = {
    ...
    try {
      ...
      (group as any).__gltfModel = model;
      return group;
    } catch {
      return null;
    }

...
export const toWorld3DX = (x) => x * WORLD_SCALE - WORLD_OFFSET_X;
export const toWorld3DZ = (z) => z * WORLD_SCALE;
export const fromWorld3DX = (x) => (x + WORLD_OFFSET_X) / WORLD_SCALE;
export const fromWorld3DZ = (z) => z / WORLD_SCALE;
...
export function inferWeaponType(item) {
    if (!item)
        return 'fist';
    if (item.weaponType)
        return item.weaponType;
    const key = `${item.id} ${item.name}`.toLowerCase();
    if (key.includes('scythe'))
        return 'scythe';
    if (key.includes('knife') || key.includes('dagger'))
        return 'dagger';
    if (key.includes('bow'))
        return 'bow';
    if (key.includes('staff') || key.includes('wand'))
        return 'staff';
    if (key.includes('axe'))
        return 'axe';
    return 'sword';
}
const ALL_JOBS = ['Novice', 'Swordman', 'Magician', 'Archer', 'Thief', 'Acolyte', 'Merchant'];
...
export function makeItem(defId, quantity = 1) {
...
export function getSellPrice(item) {
    ...
    if (def)
        return Math.max(1, Math.floor(def.price / 2) + (item.refine || 0) * 150);
    switch (item.type) {
...
export function getEquipBlocker(item, job, baseLevel) {
    if (item.type !== 'equip' || !item.slot)
        return 'ไอเทมนี้สวมใส่ไม่ได้';
    if (item.reqLevel && baseLevel < item.reqLevel)
        return `ต้องการ Base Lv.${item.reqLevel}`;
    if (item.jobs && item.jobs.length > 0 && !item.jobs.includes(job))
        return `อาชีพ ${job} ใช้ไอเทมนี้ไม่ได้`;
    return null;
}
const JOB_HP_MOD = {
...
export function computeDerived(p, now = Date.now()) {
...
const PASSIVE_MOBS = ['Sunbun', 'Poring', 'Leafkin', 'Fabre', 'Spore'];
const BOSS_MOBS = [
    'BaphometJr', 'Solarion', 'LordBaphomet', 'Valkyrie',
    'AncientPyroclastDragon', 'DemonLordMalakor', 'ColossalTitan'
];
export function isBossMonster(type) {
    return BOSS_MOBS.includes(type);
}
export function getMonsterAiProfile(type) {
...
export function getQuestState(def, progress, now = Date.now()) {
    if (!progress)
...
export const PIXEL_SLAYER_GIFT_CODES = {
    VIP777: {
...
export const SLAYER_DUNGEON_STAGES = [
    ...
    },
    {
        ...
        recommendedLv: 60,
        waves: 4,
        bossType: 'AncientPyroclastDragon',
        rewardExp: 100000,
        rewardZeny: 200000,
        rewardGems: 500
    },

...
import { PlayerData, DamageEvent, ChatMessage, Stats, JobClass, InventoryItem, CharacterModel, InputPacket, ServerSnapshot, MonsterType, ServerAnnouncement, AdminActionPacket, ZoneId, PetData, MonsterData, DropItemData } from './types.js';
import { MonsterManager, ZONES_CONFIG, combatDistance, MonsterAttackEvent } from './MonsterManager.js';
import {
  SKILL_DB, SkillDef, SKILL_MAX_LEVEL, canUseSkill, skillLevelBonus, computeDerived, DerivedStats,
  ITEM_DB, SHOPS, makeItem, getSellPrice, getEquipBlocker, inferWeaponType,
  getNpcDef, NpcAction, SERVER_NPC_ACTIONS, NPC_INTERACT_RANGE, WARP_COSTS,
  fromWorld3DX, fromWorld3DZ, TOWN_RESPAWN, WORLD_MAX_X, WORLD_MAX_Z, WORLD_SCALE,
  QUEST_DB, getQuestState, PIXEL_SLAYER_GIFT_CODES
} from './GameData.js';

const STAT_KEYS: Array<keyof Stats> = ['str', 'agi', 'vit', 'int', 'dex', 'luk'];
const CHAT_MIN_INTERVAL_MS = 600;

const VALID_JOBS: JobClass[] = ['Swordman', 'Magician', 'Archer', 'Thief', 'Acolyte', 'Merchant'];
...
export class GameRoom {
  public clients: Map<WebSocket, PlayerData> = new Map();
  public monsterManager = new MonsterManager();
  public aiEngine: AetherAiEngine;
  private updateInterval: NodeJS.Timeout;
  private lastAttackTimes: Map<string, number> = new Map();
  /** `${playerId}:${skillId}` -> timestamp when the skill is ready again */
  private skillCooldowns: Map<string, number> = new Map();
  private lastPosSyncAt: Map<string, number> = new Map();
  ...
  public addClient(ws: WebSocket, character: CharacterModel): PlayerData {
    ...
    if (player.hp <= 0) player.hp = Math.floor(player.maxHp * 0.5);
  ...
  public removeClient(ws: WebSocket) {
    ...
    if (player) {
      ...
      for (const key of this.skillCooldowns.keys()) {
        if (key.startsWith(`${player.id}:`)) this.skillCooldowns.delete(key);
      }
      // Monsters chasing this player lose interest
      this.monsterManager.monsters.forEach(m => {
        if (m.targetPlayerId === player.id) m.targetPlayerId = undefined;
      });
    }
  }

  public persistPlayer(player: PlayerData) {
  ...
  private publicPlayer(p: PlayerData) {
    return {
      id: p.id,
      name: p.name,
      job: p.job,
  ...
  private getSocket(player: PlayerData): WebSocket | undefined {
    for (const [ws, p] of this.clients) {
      if (p === player) return ws;
    }
    return undefined;
  }

  /** Full character state goes only to its owner */
  private sendPlayerUpdate(player: PlayerData, leveledUp?: boolean) {
  ...
  public kickCharacter(characterId: string) {
  ...
  public handleMessage(ws: WebSocket, message: any) {
    const player = this.clients.get(ws);
    if (!player) return;
    ...
    switch (message.type) {
      ...
      case 'INPUT': {
        const packet = message.packet as InputPacket;
        // The 3D client is position-synced via POS_SYNC_3D; dead players can't move
        if (player.isDead || player.viewMode === '3d') break;
      ...
      case 'POS_SYNC_3D': {
        ...
        if (!Number.isFinite(sx) || !Number.isFinite(sz)) break;
        ...
        player.z = nz;
        player.y = 560;
        if (Number.isFinite(Number(message.rotY))) player.rotY = Number(message.rotY);
        if (message.anim === 'walk' || message.anim === 'idle' || message.anim === 'jump') player.anim = message.anim;
        if (message.facing === 'left' || message.facing === 'right') player.facing = message.facing;
        break;
      }
      ...
      case 'NPC_ACTION': {
        this.handleNpcAction(ws, player, String(message.npcId || ''), String(message.action || '') as NpcAction, message.param ? String(message.param) : undefined);
        break;
      }
      ...
      case 'ALLOCATE_STAT': {
        const stat = message.stat as keyof Stats;
      ...
      case 'JOB_CHANGE': {
        const newJob = message.job as JobClass;
        if (!VALID_JOBS.includes(newJob)) break;
      ...
      case 'PICK_DROP': {
        ...
        const candidates: DropItemData[] = message.dropId
          ? [this.monsterManager.drops.get(String(message.dropId))].filter((d): d is DropItemData => Boolean(d))
        ...
        if (picked.length === 0) {
          if (!message.dropId) this.sendChat(ws, 'system', '🔍 ไม่มีไอเทมอยู่ใกล้ตัว');
          break;
        }
      ...
      case 'CHAT': {
        ...
        if (text.length === 0 || text.length > 150) break;
        if (text.startsWith('@')) {
      ...
      case 'AI_ACTION': {
        if (message.action !== 'GET_STATE' && !player.isGm) break;
      ...
      case 'UPGRADE_SKILL': {
        ...
        if (!skill || skill.id === 'NORMAL') { this.sendChat(ws, 'system', '❌ ไม่พบสกิลที่ต้องการ'); break; }
        if (!canUseSkill(skill, player.job)) { this.sendChat(ws, 'system', `❌ อาชีพ ${player.job} เรียนสกิลนี้ไม่ได้`); break; }
        if (!player.skillPoints || player.skillPoints <= 0) { this.sendChat(ws, 'system', '❌ แต้มสกิลไม่พอ'); break; }
        if (!player.skillLevels) player.skillLevels = {};
        const curLv = player.skillLevels[skillId] || 1;
        if (curLv >= SKILL_MAX_LEVEL) { this.sendChat(ws, 'system', `✨ สกิล [${skill.name}] Lv.${SKILL_MAX_LEVEL} สูงสุดแล้ว!`); break; }
      ...
      case 'PET_TOGGLE': {
        if (player.activePet) {
          ...
          player.activePet = { id: `pet_${Date.now()}`, name: 'Chibi Sunbun', type: 'sunbun', level: 1, autoLoot: true, buff: { speedBoost: 0.1, expBonus: 0.05 } } as any;
  ...
  private handleGmChatCommand(ws: WebSocket, player: PlayerData, text: string) {
    if (!player.isGm && player.role !== 'admin' && player.role !== 'gm') {
      this.sendChat(ws, 'system', '❌ คุณไม่มีสิทธิ์ใช้งานคำสั่งผู้ดูแลระบบ');
      return;
    }
    ...
    switch (cmd) {
      ...
      case 'spawn': {
        const mobType = (args[0] || 'Solarion') as MonsterType;
      ...
      case 'job': {
        const targetJob = (args[0] || 'Novice') as JobClass;
  ...
  private handleAdminAction(ws: WebSocket, player: PlayerData, action: AdminActionPacket['action'], data: any = {}) {
    switch (action) {
      case 'BROADCAST': {
      ...
      case 'SPAWN_MOB': {
        const mobType = (data.mobType || 'Solarion') as MonsterType;
  ...
  private grantGmItem(ws: WebSocket, player: PlayerData, itemKey: string) {
  ...
  private spawnMobsForGm(ws: WebSocket, player: PlayerData, mobType: MonsterType, count: number) {
    const validMobs: Record<string, string> = {
      Sunbun: 'Sunbun',
      Leafkin: 'Leafkin',
      Aethercap: 'Aethercap',
      Gryphlet: 'Gryphlet',
    ...
    const chosenType = typeKey as MonsterType;
    ...
    for (let i = 0; i < count; i++) {
      ...
      const spawned = this.monsterManager.spawnMonster(
        chosenType,
        mobName,
        spawnX,
        spawnY,
        Math.max(100, spawnX - 250),
        Math.min(14350, spawnX + 250)
      );
  ...
  private killAllMonsters(ws: WebSocket, player: PlayerData) {
    let count = 0;
  ...
  public broadcastAnnouncement(message: string, sender: string = 'Game Master', style: 'gold' | 'crimson' | 'emerald' | 'cyan' = 'gold') {
  ...
  private async handleRefineItem(ws: WebSocket, player: PlayerData, itemId: string) {
    if (!player.characterId) return;
  ...
  private sendSkillFailed(ws: WebSocket, skillId: string, reason: string) {
  ...
  private sendVitals(ws: WebSocket, player: PlayerData) {
    if (ws.readyState === WebSocket.OPEN) {
      ws.send(JSON.stringify({
        type: 'VITALS',
  ...
  private sendPosCorrection(ws: WebSocket, player: PlayerData, x: number, z: number) {
    player.x = x;
    player.z = z;
    if (ws.readyState === WebSocket.OPEN) {
  ...
  private isValidTarget(mob: MonsterData | undefined): mob is MonsterData {
    return Boolean(mob && !mob.isDead && mob.hp > 0);
  }

  /** Requested target if it's in range, otherwise the closest living monster in range */
  private pickTarget(player: PlayerData, range: number, targetId?: string): MonsterData | undefined {
    ...
    let bestDist = range;
    this.monsterManager.monsters.forEach(m => {
      if (!this.isValidTarget(m)) return;
  ...
  private rollDamage(skill: SkillDef, derived: DerivedStats, levelBonus: number, mob: MonsterData): { damage: number; isCrit: boolean } {
  ...
  private handleSkill(ws: WebSocket, player: PlayerData, skillId: string, targetId?: string) {
    ...
    if (now < (this.skillCooldowns.get(cdKey) || 0) - 80) return;
    ...
    // --- Resolve targets ---
    let primary: MonsterData | undefined;
    let victims: MonsterData[] = [];
    let center = { x: player.x, y: player.y, z: player.z || 0 };

    const inRadius = (c: { x: number; y: number; z: number }, radius: number) => {
      const list: MonsterData[] = [];
      this.monsterManager.monsters.forEach(m => {
        if (this.isValidTarget(m) && combatDistance({ ...c, viewMode: player.viewMode }, m) <= radius) list.push(m);
      });
      return list;
    };
    ...
    switch (skill.kind) {
      ...
      case 'blink': {
        ...
        if (!primary) {
          ...
          if (skill.kind !== 'basic') this.sendSkillFailed(ws, skillId, 'ไม่มีเป้าหมายในระยะ');
          return;
        }
        victims = [primary];
        break;
    ...
    if (victims.length > 0) player.lastCombatAt = now;
    ...
    if (skill.healPct) {
      ...
      const healTargets = skill.id === 'SANCTUARY'
        ...
        : [[ws, player] as [WebSocket, PlayerData]];
  ...
  private killMonster(killer: PlayerData, mob: MonsterData) {
    const drops = this.monsterManager.handleMobDeath(mob);
    this.broadcast({
      type: 'MOB_DIED',
      mobId: mob.id,
  ...
  private creditQuestKill(player: PlayerData, mobType: MonsterType) {
    const now = Date.now();
    if (!player.questProgress) player.questProgress = {};
    let changed = false;
    for (const def of QUEST_DB) {
      if (!def.targets.includes(mobType)) continue;
  ...
  private handleClaimQuest(ws: WebSocket, player: PlayerData, questId: string) {
    const def = QUEST_DB.find(q => q.id === questId);
    if (!def || player.isDead) return;
    if (!player.questProgress) player.questProgress = {};
    ...
    for (const reward of def.rewardItems) {
      ...
      if (item) this.addItemToInventory(player, item);
    }
  ...
  private resolveMonsterAttacks(attacks: MonsterAttackEvent[], now: number) {
    if (attacks.length === 0) return;
    const byId = new Map<string, [WebSocket, PlayerData]>();
    ...
    for (const atk of attacks) {
      ...
      if (!mob || mob.isDead || !entry) continue;
  ...
  private killPlayer(ws: WebSocket, player: PlayerData, killer: MonsterData) {
    player.isDead = true;
    player.anim = 'dead';
    player.hp = 0;
  ...
  private handleRespawn(ws: WebSocket, player: PlayerData) {
    if (!player.isDead) return;
    player.isDead = false;
    player.anim = 'idle';
  ...
  private addItemToInventory(player: PlayerData, item: InventoryItem) {
    if (item.type !== 'equip') {
      const existing = player.inventory.find(i =>
        i.type === item.type && (item.defId ? i.defId === item.defId || i.name === item.name : i.name === item.name)
      );
      if (existing) {
  ...
  private collectDrop(ws: WebSocket, player: PlayerData, drop: DropItemData) {
    if (drop.type === 'zeny') {
      ...
      const item = dbDef
        ...
        : {
            ...
            weaponType: drop.itemData.slot === 'weapon' ? inferWeaponType(drop.itemData as InventoryItem) : undefined,
            rarity: drop.itemData.type === 'equip' ? 'legendary' as const : undefined,
  ...
  private handleEquip(ws: WebSocket, player: PlayerData, itemId: string) {
    if (player.isDead) return;
    ...
    if (slot === 'weapon' && !item.weaponType) item.weaponType = inferWeaponType(item);
  ...
  private handleUnequip(ws: WebSocket, player: PlayerData, slot: string) {
    if (player.isDead) return;
    const validSlots = ['weapon', 'head', 'armor', 'shoes', 'accessory'] as const;
  ...
  private isNearNpc(player: PlayerData, npcX3d: number, npcZ3d: number): boolean {
    // NPCs only exist in the 3D town
    if (player.viewMode !== '3d') return false;
    return Math.hypot(player.x - fromWorld3DX(npcX3d), (player.z || 0) - fromWorld3DZ(npcZ3d)) <= NPC_INTERACT_RANGE;
  }

  private handleNpcAction(ws: WebSocket, player: PlayerData, npcId: string, action: NpcAction, param?: string) {
    const npc = getNpcDef(npcId);
    if (!npc || player.isDead) return;
    const option = npc.options.find(o => o.action === action && (o.param || '') === (param || ''));
    if (!option || !SERVER_NPC_ACTIONS.includes(action)) return;
  ...
  private priceModifier(player: PlayerData, selling: boolean): number {
    if (player.job !== 'Merchant') return 1;
    return selling ? 1.1 : 0.9;
  }

  private handleShopBuy(ws: WebSocket, player: PlayerData, npcId: string, shopId: string, itemId: string, qtyRaw: number) {
    const npc = getNpcDef(npcId);
    const shop = SHOPS[shopId];
    const def = ITEM_DB[itemId];
    const sendResult = (ok: boolean, message: string) => ws.send(JSON.stringify({ type: 'SHOP_RESULT', ok, message }));
    if (!npc || !shop || !def || player.isDead) return;
    if (!npc.options.some(o => o.action === 'OPEN_SHOP' && o.param === shopId) || !shop.items.includes(itemId)) return;
    if (!this.isNearNpc(player, npc.x, npc.z)) {
    ...
    player.zeny -= total;
    if (def.type === 'equip') {
      for (let i = 0; i < qty; i++) this.addItemToInventory(player, makeItem(itemId, 1)!);
    } else {
  ...
  private handleShopSell(ws: WebSocket, player: PlayerData, npcId: string, invItemId: string, qtyRaw: number) {
    const npc = getNpcDef(npcId);
    const sendResult = (ok: boolean, message: string) => ws.send(JSON.stringify({ type: 'SHOP_RESULT', ok, message }));
    if (!npc || player.isDead || !npc.options.some(o => o.action === 'OPEN_SHOP')) return;
    if (!this.isNearNpc(player, npc.x, npc.z)) {
      sendResult(false, `เดินเข้าไปใกล้ ${npc.name} อีกนิด`);
      return;
    }
  ...
  private handleUseItem(player: PlayerData, itemId: string) {
  ...
  private awardExp(player: PlayerData, baseExp: number, jobExp: number) {
  ...
  private recalculatePlayerStats(player: PlayerData) {
  ...
  private tick() {
    ...
    const playersList = players.map(p => ({
      ...
      vy: Math.round(p.vy),
      facing: p.facing,
      anim: p.isDead ? 'dead' as const : p.anim,
      hp: p.hp,
      maxHp: p.maxHp,
      gender: p.gender,
      hairColor: p.hairColor,
  ...
  public broadcast(data: any, exceptWs?: WebSocket) {
  ...
  public sendChat(ws: WebSocket, channel: ChatMessage['channel'], text: string) {
    if (ws.readyState === WebSocket.OPEN) {
      ws.send(JSON.stringify({
        type: 'CHAT',
        chat: {
  ...
  public sendSystemChat(text: string) {
    this.broadcast({
      type: 'CHAT',
      chat: {

...
export class WorldScene extends Phaser.Scene {
    ...
    interpBuffer = new InterpolationBuffer();
    assetStreamer;
    questManager;
    botEngine;
    // Phaser Game Objects
    ...
    // Entities
    selfData = null;
    remotePlayerContainers = new Map();
    monsterContainers = new Map();
    dropContainers = new Map();
    // Controls
    cursors;
    keyW;
    keyA;
    ...
    drawPlayerVisuals(container, name, job, baseLevel, isSelf, isGm) {
    ...
    setupQuickslotClicks() {
        ...
        document.getElementById('slot-1')?.addEventListener('click', () => {
            if (this.threeWorld)
                this.threeWorld.usePotion('hp');
        });
        document.getElementById('slot-2')?.addEventListener('click', () => {
            if (this.threeWorld)
                this.threeWorld.usePotion('mp');
        });
    ...
    setupNetwork() {
        this.network = new NetworkClient({
            ...
            onDropRemoved: (dropId, pickerId) => {
                ...
                if (drop) {
                    if (this.selfData && pickerId === this.selfData.id)
                        sound.playCoin();
    ...
    spawnRemotePlayer(p) {
        if (this.remotePlayerContainers.has(p.id))
            return;
    ...
    spawnMonster(m) {
        if (this.monsterContainers.has(m.id))
            return;
        ...
        if (m.type === 'Sunbun')
            tex = 'sunbun';
        else if (m.type === 'Leafkin')
            tex = 'leafkin';
        else if (m.type === 'Aethercap')
            tex = 'aethercap';
        else if (m.type === 'Gryphlet')
            tex = 'gryphlet';
        else if (m.type === 'Solarion') {
            tex = 'solarion';
            scale = 1.25;
        }
        else if (m.type === 'Fabre')
            tex = 'fabre';
        else if (m.type === 'Spore')
            tex = 'spore';
        else if (m.type === 'BaphometJr')
            tex = 'baphomet';
    ...
    spawnDrop(d) {
        if (this.dropContainers.has(d.id))
            return;
    ...
    update(time, delta) {
        ...
        const predicted = this.prediction.predict(this.selfData, inputState, dt, this.platforms);
        ...
        monsters.forEach((m, id) => {
            ...
            if (container) {
                ...
                if (m.isDead !== undefined)
                    container.setVisible(!m.isDead);
            }
        });
    ...
    getJobPrimarySkill(job) {
        return JOB_SKILL_BAR[job][0];
    }
    getJobSecondarySkill(job) {
    ...
    showSkillEffect(x, y, isLeft, skill = 'NORMAL') {
        ...
        if (skill === 'RADIANT_SLASH') {
            ...
            g.fillCircle(x + dir * 70, y - 80, 22);
            g.fillStyle(0xffd166, 0.7);
            g.fillCircle(x + dir * 70, y - 80, 14);
            g.setDepth(20);
            ...
            ring.lineStyle(4, 0xff7f00, 0.8);
            ring.strokeCircle(x + dir * 70, y + 20, 10);
            ring.setDepth(21);
            ...
            g.fillStyle(0xffffff, 0.5);
            g.fillEllipse(x + dir * 56, y, 60, 10);
            g.setDepth(20);
            ...
            // Dark teleport flash
            const g = this.add.graphics();
            g.fillStyle(0x7209b7, 0.9);
            g.fillCircle(x, y - 15, 40);
            g.fillStyle(0x000000, 0.5);
            g.fillCircle(x, y - 15, 24);
            g.setDepth(20);
            ...
            for (let i = 0; i < 5; i++) {
                this.time.delayedCall(i * 60, () => {
                    ...
                    g.fillStyle(0x9d4edd, 0.85);
                    g.fillTriangle(sx, sy - 14, sx + dir * 26, sy - 4, sx + dir * 26, sy + 4);
                    g.setDepth(20);
            ...
            g.fillEllipse(x, y + 20, 100, 30);
            g.lineStyle(4, 0xffd700, 0.9);
            g.strokeEllipse(x, y + 20, 100, 30);
            ...
            g.fillCircle(x + dir * 50, y - 10, 28);
            g.fillStyle(0xff6d00, 0.7);
            g.fillCircle(x + dir * 50, y - 10, 18);
            g.setDepth(20);
            ...
            g.strokeCircle(x, y - 10, 60);
            g.fillStyle(0xffd700, 0.25);
            g.fillCircle(x, y - 10, 60);
            g.setDepth(20);
            ...
            // Shield bubble
            const g = this.add.graphics();
            g.lineStyle(6, 0xffd166, 0.9);
            g.strokeCircle(x, y - 20, 42);
            g.fillStyle(0xffd166, 0.2);
            g.fillCircle(x, y - 20, 42);
            g.setDepth(20);
            ...
            const g = this.add.graphics();
            g.fillStyle(0xff4e00, 0.85);
            g.fillTriangle(x + dir * 15, y - 30, x + dir * 65, y - 15, x + dir * 65, y + 10);
            g.fillStyle(0xffffff, 0.4);
            g.fillCircle(x + dir * 45, y - 12, 16);
            g.setDepth(20);
    ...
    showLevelUpEffect(x, y, title = '★ LEVEL UP! ★') {

...
export type HeroElement = 'fire' | 'water' | 'earth' | 'wind' | 'light' | 'shadow';
export type HeroRole = 'Knight' | 'Mage' | 'Rogue' | 'Assassin' | 'Support' | 'Berserker' | 'Boss';
...
export interface HeroSkill {
  ...
  cooldownMs: number;
  multiplier: number;
  effectKind: 'slash' | 'meteor' | 'ice' | 'heal' | 'buff' | 'chains' | 'beam' | 'barrier' | 'whirlwind';
  description: string;
}
...
export interface HeroDef {
  ...
  thaiName: string;
  title: string;
  rarity: 3 | 4 | 5 | 6; // 6 = UR, 5 = SSR, 4 = SR, 3 = R
  element: HeroElement;
  role: HeroRole;
  baseHp: number;
  baseAtk: number;
  baseDef: number;
  aspd: number; // Attack interval in seconds
  range: number; // Combat range in 3D world units
  signatureSkill: HeroSkill;
  spriteUrl: string;
  cardUrl: string;
  iconUrl: string;
  colorTheme: string;
...
export const HERO_ROSTER_DB: Record<string, HeroDef> = {
  sutmes: {
    id: 'sutmes',
    name: 'Sutmes',
    thaiName: 'ซุตเมส',
...
export class HeroRosterManager {
  private static instance: HeroRosterManager;
  private unlockedHeroIds: Set<string> = new Set();
  private partySlots: [string, string | null, string | null, string | null] = ['player', null, null, null];
  private summonCurrency: number = 1000; // Starting summon crystals / gems

  private constructor() {
    this.loadState();
  }

  public static getInstance(): HeroRosterManager {
  ...
  public getUnlockedHeroes(): HeroDef[] {
  ...
  public getParty(): [string, string | null, string | null, string | null] {
    return [...this.partySlots];
  }

  public getPartyHeroes(): Array<HeroDef | null> {
    return this.partySlots.map(id => (id && id !== 'player' ? HERO_ROSTER_DB[id] || null : null));
  }

  public setPartySlot(slotIndex: 1 | 2 | 3, heroId: string | null) {
   ...
   */
  public summon(is10x: boolean = false): { results: Array<{ hero: HeroDef; isNew: boolean }>; success: boolean; error?: string } {
    ...
    const results: Array<{ hero: HeroDef; isNew: boolean }> = [];
    const allHeroIds = Object.keys(HERO_ROSTER_DB);

    for (let i = 0; i < count; i++) {
      const rand = Math.random() * 100;
      let targetRarity: 3 | 4 | 5 | 6 = 3;

      if (is10x && i === count - 1) {
        // Guaranteed 5★+ on 10th summon
        targetRarity = rand < 25 ? 6 : 5;
      } else {
        if (rand < 5) targetRarity = 6;      // 5% UR (Flora, Krochid, Lakhia)
        else if (rand < 30) targetRarity = 5; // 25% SSR
        else if (rand < 70) targetRarity = 4; // 40% SR
        else targetRarity = 3;                // 30% R
      }
   ...
   */
  public getPartySynergy(): PartySynergy {
    const heroes = this.getPartyHeroes().filter((h): h is HeroDef => h !== null);
    let bonusAtk = 0;
    let bonusDef = 0;
    let bonusRegen = 0;
    const notes: string[] = [];

...
export class SkillIconManager {
    ...
    renderSlotIcon(element, skillId, count) {
    ...
    updateJobSkills(job) {
        ...
        const [skill1, skill2] = JOB_SKILL_BAR[job] || JOB_SKILL_BAR.Novice;
        if (slot4)
            this.renderSlotIcon(slot4, skill1);
        const slot5Root = document.getElementById('slot-5');
        if (slot5Root)
            slot5Root.style.visibility = skill2 ? 'visible' : 'hidden';
        if (slot5 && skill2)
            this.renderSlotIcon(slot5, skill2);
        const setTitle = (id, skillId, key) => {
            const el = document.getElementById(id);
            const def = skillId ? SKILL_DB[skillId] : undefined;
            if (el && def)
                el.title = `${def.name} (${def.thaiName}) [${key}] — ${def.mpCost} MP`;
        };
        setTitle('slot-4', skill1, '4');
        setTitle('slot-5', skill2, '5');
        // Mobile action buttons show the job's real skills
        const setMobileButton = (id, skillId) => {

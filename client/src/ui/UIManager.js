...
export class UIManager {
    ...
    enableGm(isGm) {
        ...
        if (gmTopBtn)
            gmTopBtn.style.display = isGm ? 'flex' : 'none';
        if (gmBtmBtn)
            gmBtmBtn.style.display = isGm ? 'flex' : 'none';
        if (gmDrawerBtn)
            gmDrawerBtn.style.display = isGm ? 'flex' : 'none';
    ...
    updateBotStatus(running) {
        ...
        if (running) {
            ...
            if (toggleBtn)
                toggleBtn.textContent = 'ปิดทำงาน (OFF)';
            if (tbLabel)
                tbLabel.textContent = 'Bot: ON';
            if (tbBtn)
                tbBtn.style.background = 'rgba(56, 176, 0, 0.35)';
        }
        else {
            if (statusText) {
            ...
            if (toggleBtn)
                toggleBtn.textContent = 'เปิดทำงาน (ON)';
            if (tbLabel)
                tbLabel.textContent = 'Bot: OFF';
            if (tbBtn)
                tbBtn.style.background = 'rgba(217, 4, 41, 0.25)';
        }
    }
    renderQuestJournal(qm) {
    ...
    setupQuestAndBotControls() {
        ...
        tabBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                this.activeQuestTab = (target.getAttribute('data-qtab') || 'active');
        ...
        const chkAttack = document.getElementById('bot-chk-attack');
        const chkSkill = document.getElementById('bot-chk-skill');
        const chkPotion = document.getElementById('bot-chk-potion');
        const chkLoot = document.getElementById('bot-chk-loot');
        chkAttack?.addEventListener('change', () => { if (this.botEngine)
            this.botEngine.settings.autoAttack = chkAttack.checked; });
        chkSkill?.addEventListener('change', () => { if (this.botEngine)
            this.botEngine.settings.autoSkill = chkSkill.checked; });
        chkPotion?.addEventListener('change', () => { if (this.botEngine)
            this.botEngine.settings.autoPotion = chkPotion.checked; });
        chkLoot?.addEventListener('change', () => { if (this.botEngine)
            this.botEngine.settings.autoLoot = chkLoot.checked; });
    }
    renderMiniMap(self, players, monsters, mapW, mapH) {
    ...
    updatePlayerHUD(player) {
        ...
        if (jobBadge)
            jobBadge.textContent = `${player.job} (Lv.${player.baseLevel})`;
        const zenyEl = document.getElementById('hud-zeny');
        if (zenyEl)
            zenyEl.textContent = `${player.zeny.toLocaleString()} Z`;
        // HP Bar
        ...
        if (baseFill)
            baseFill.style.width = `${baseRatio * 100}%`;
        if (baseText)
            baseText.textContent = `Base EXP: ${(baseRatio * 100).toFixed(1)}%`;
        // Job EXP Bar
        ...
        if (jobFill)
            jobFill.style.width = `${jobRatio * 100}%`;
        if (jobText)
            jobText.textContent = `Job EXP: ${(jobRatio * 100).toFixed(1)}% (J.Lv.${player.jobLevel})`;
        // Update Windows
    ...
    updateRefineWindow(player) {
        ...
        if (weapon) {
            ...
            const curRefine = weapon.refine || 0;
            const cost = (curRefine + 1) * 250;
            if (nameEl)
                nameEl.textContent = weapon.name;
            if (levelEl)
                levelEl.textContent = `+${curRefine} (ATK +${weapon.effect?.atk || 0})`;
            if (costEl)
                costEl.textContent = `${cost.toLocaleString()} Zeny`;
            if (btn) {
            ...
            if (nameEl)
                nameEl.textContent = 'ไม่มีอาวุธที่สวมใส่';
            if (levelEl)
                levelEl.textContent = '-';
            if (costEl)
                costEl.textContent = '-';
    ...
    updateStatusWindow(player) {
        const ptsEl = document.getElementById('stat-points-val');
        if (ptsEl)
            ptsEl.textContent = `${player.statPoints}`;
        const stats = ['str', 'agi', 'vit', 'int', 'dex', 'luk'];
        stats.forEach(s => {
            const valEl = document.getElementById(`stat-${s}-val`);
            if (valEl)
                valEl.textContent = `${player.stats[s]}`;
            const btn = document.getElementById(`stat-${s}-btn`);
            if (btn) {
    ...
    updateQuickslotCounts(player) {
        ...
        if (s1)
            s1.textContent = red ? `${red.quantity}` : '0';
        if (s2)
            s2.textContent = blue ? `${blue.quantity}` : '0';
        if (s3)
            s3.textContent = wing ? `${wing.quantity}` : '0';
        const mp1 = document.getElementById('m-potion1-count');
        const mp2 = document.getElementById('m-potion2-count');
        if (mp1)
            mp1.textContent = red ? `${red.quantity}` : '0';
        if (mp2)
            mp2.textContent = blue ? `${blue.quantity}` : '0';
    }
    appendChat(chat) {
    ...
    addChatMessage(sender, text, channel = 'system') {
    ...
    setupEventListeners() {
        ...
        document.getElementById('btn-sound')?.addEventListener('click', () => {
            ...
            if (btn)
                btn.textContent = isMuted ? '🔇 เสียง: ปิด' : '🔊 เสียง: เปิด';
        });
        ...
        btnMobileToggle?.addEventListener('click', () => {
            ...
            if (small)
                small.textContent = isTouch ? 'Touch: ON' : 'Touch';
        });
    ...
    setupHeroModals() {
        ...
        const updateTowerPreview = (floor) => {
      ...
      `;
        ...
        document.querySelectorAll('.tower-floor-item').forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                selectedFloor = parseInt(e.currentTarget.getAttribute('data-floor') || '1', 10);
                updateTowerPreview(selectedFloor);
            });
        });
        ...
        document.getElementById('btn-redeem-gift')?.addEventListener('click', () => {
            ...
            if (!code) {
                if (statusEl) {
                    statusEl.textContent = '⚠️ กรุณากรอกโค้ดก่อนครับ';
            ...
            if (!this.network) {
                if (statusEl) {
                    statusEl.textContent = '❌ ยังไม่ได้เชื่อมต่อเซิร์ฟเวอร์';
            ...
            if (statusEl) {
                statusEl.textContent = '⏳ กำลังตรวจสอบโค้ด...';
    ...
    setupGiftCodeNetworkListener() {
        ...
        this.network.on('GIFT_CODE_RESULT', (msg) => {
            ...
            if (msg.success) {
                if (statusEl) {
                    statusEl.textContent = `✅ แลกรับ [${msg.name}] สำเร็จ! +${msg.gems} Gems, +${msg.zeny?.toLocaleString()} Zeny`;
                ...
                if (statusEl) {
                    statusEl.textContent = `❌ ${msg.reason || 'โค้ดไม่ถูกต้องหรือถูกใช้ไปแล้ว'}`;
                    statusEl.style.color = '#f87171';
                }
            }
        });
    }
    updateSummonCurrency() {
    ...
    updateBossRaidBar(show, name, hp, maxHp, bossType = '') {
        ...
        if (iconEl)
            iconEl.textContent = iconMap[bossType] || '👾';
        if (nameEl)
            nameEl.textContent = name;
        if (phaseEl) {
            phaseEl.textContent = phase;
            phaseEl.style.background = pct < 50 ? 'linear-gradient(90deg,#dc2626,#7f1d1d)' : 'linear-gradient(90deg,#b91c1c,#7f1d1d)';
        }
        if (fillEl) {
        ...
        if (pctEl)
            pctEl.textContent = `${Math.round(pct)}% (${hp.toLocaleString()} / ${maxHp.toLocaleString()})`;
    }
    executeSummon(is10x) {
        const roster = HeroRosterManager.getInstance();
    ...
    renderPartyModal() {
        ...
        for (let slot = 1; slot <= 3; slot++) {
            ...
            if (heroId && HERO_ROSTER_DB[heroId]) {
                ...
                if (avatarEl)
                    avatarEl.innerHTML = '<span style="font-size: 20px; color: #64748b;">+</span>';
                if (nameEl)
                    nameEl.textContent = 'ว่าง (แตะด้านล่าง)';
                if (roleEl)
                    roleEl.textContent = '-';
        ...
        document.querySelectorAll('.btn-remove-slot').forEach(btn => {
            ...
            newBtn.addEventListener('click', (e) => {
                ...
                if (slotIdx >= 1 && slotIdx <= 3) {
                    roster.setPartySlot(slotIdx, null);
                    this.renderPartyModal();
                    if (this.worldScene?.threeWorld) {
                        this.worldScene.threeWorld.refreshCompanions();
                    }
    ...
    setupAdminControls() {
        ...
        document.querySelectorAll('.gm-speed-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (valEl)
                    valEl.textContent = `${spd.toFixed(1)}x`;
            });
        });
        ...
        document.getElementById('gm-broadcast-send-btn')?.addEventListener('click', () => {
            const input = document.getElementById('gm-broadcast-input');
            const styleSelect = document.getElementById('gm-broadcast-style');
            const msg = input?.value?.trim();
            if (msg) {
                this.adminManager?.broadcast(msg, (styleSelect?.value || 'gold'));
                input.value = '';
            }
        });
        document.getElementById('gm-spawn-mob-btn')?.addEventListener('click', () => {
            const select = document.getElementById('gm-spawn-select');
            const countInput = document.getElementById('gm-spawn-count');
        ...
        document.querySelectorAll('.gm-zeny-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const amount = parseInt(e.currentTarget.getAttribute('data-zeny') || '100000', 10);
        ...
        document.querySelectorAll('.gm-item-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const itemKey = e.currentTarget.getAttribute('data-item') || 'excalibur';
    ...
    setup3DModelStudio() {
      ...
      `).join('');
        ...
        const inputUrl = document.getElementById('ai-model-custom-url');
    ...
    updateAiCore(state) {
        ...
        if (cyclesEl)
            cyclesEl.textContent = String(state.cyclesCompleted);
        // Progress
        const progressText = document.getElementById('ai-learning-progress-text');
        if (progressText)
            progressText.textContent = `${Math.round(state.learningProgress)}%`;
        const progressFill = document.getElementById('ai-learning-progress-fill');
        if (progressFill)
            progressFill.style.width = `${Math.min(100, Math.max(0, state.learningProgress))}%`;
        // Anomaly score
    ...
    setupWindowDragging() {
        ...
        windows.forEach(win => {
            const header = win.querySelector('.ro-window-header');
            if (!header)
                return;
            ...
            header.addEventListener('mousedown', (e) => {
                ...
                initialTop = rect.top;
                document.querySelectorAll('.ro-window').forEach(w => w.style.zIndex = '100');

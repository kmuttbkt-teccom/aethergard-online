import { PlayerData, JobClass, InventoryItem } from '../../../server/src/types.js';
...
export class ClientPlayer {
  ...
  public attackSkill: string = 'NORMAL';
  public flashJumpPuff: { x: number; y: number; timer: number } | null = null;
  public levelUpEffect: { timer: number } | null = null;

  // Chat bubble
  public chatBubble: { text: string; timer: number } | null = null;

  // Animation timers
  private animTimer: number = 0;
  private blinkTimer: number = 2.0;

  constructor(data: PlayerData, isSelf: boolean = false) {
    this.data = data;
    this.isSelf = isSelf;
    this.renderX = data.x;
    this.renderY = data.y;
  ...
  public updatePhysics(
    dt: number,
    map: GameMap,
    keys: { left: boolean; right: boolean; up: boolean; down: boolean; jump: boolean; dropDown: boolean }
  ) {
    if (!this.isSelf) {
    ...
    // Check ladders
    const nearLadder = map.ladders.find(
      l => Math.abs(this.data.x - l.x) < 20 && this.data.y >= l.topY - 10 && this.data.y <= l.bottomY + 10
    );
    ...
    if (this.isClimbing) {
      ...
      // Gravity
      this.data.vy += 980 * dt;
      if (this.data.vy > 650) this.data.vy = 650;
  ...
  public triggerAttack(skill: string = 'NORMAL') {
  ...
  private renderPaperdoll(ctx: CanvasRenderingContext2D) {
    ...
    if (this.data.job === 'Swordman') bodyColor = '#d90429';
    if (this.data.job === 'Magician') bodyColor = '#7209b7';
    if (this.data.job === 'Archer') bodyColor = '#2b9348';
    if (this.data.job === 'Thief') bodyColor = '#212529';
    if (this.data.job === 'Acolyte') bodyColor = '#ffd166';
    if (this.data.job === 'Merchant') bodyColor = '#f77f00';
  ...
  private renderAttackSwipe(ctx: CanvasRenderingContext2D) {
  ...
  private renderLevelUpAnimation(ctx: CanvasRenderingContext2D) {
  ...
  private renderOverheadInfo(ctx: CanvasRenderingContext2D) {
    ctx.save();

/**
 * AETHERGARD ONLINE — GameplayUI
 * Equipment & inventory, NPC shops, the skill tree, hotbar cooldown sweeps,
 * the target frame and the death screen. All actions go to the server; the
 * UI only re-renders from the PlayerData the server sends back.
 */
import { InventoryItem, JobClass, PlayerData } from '../../../server/src/types.js';
import {
  ITEM_DB, SHOPS, SKILL_DB, SKILL_MAX_LEVEL, JOB_SKILL_BAR, RARITY_COLORS, WEAPON_PROFILES,
  getEquipBlocker, getSellPrice, getJobSkills, inferWeaponType, computeDerived, skillLevelBonus, ItemDef
...
export interface GameplayActions {
  ...
  buy(npcId: string, shopId: string, itemId: string, qty: number): void;
  sell(npcId: string, invItemId: string, qty: number): void;
  respawn(): void;
}

const EQUIP_SLOTS: Array<{ slot: 'weapon' | 'armor' | 'head' | 'shoes' | 'accessory'; label: string; icon: string }> = [
...
function describeEffect(effect?: InventoryItem['effect']): string {
  if (!effect) return '';
...
export class GameplayUI {
  ...
  private shop: { npcId: string; shopId: string; tab: 'buy' | 'sell' } | null = null;
  private cooldowns: Map<string, { start: number; end: number }> = new Map();
  private cooldownFrame = 0;

  constructor(private actions: GameplayActions) {
    this.injectStyles();
  ...
  public getHotbarSkills(job: JobClass): Record<string, string | null> {
   ...
   */
  public updateWeaponSkillBar(skills: [string, string, string, string], weaponName: string, dripColor: string) {
    ...
    if (btnBash) {
      ...
      if (small && sk) small.textContent = sk.name.slice(0, 8);
    }
  ...
  private renderEquipment(player: PlayerData) {
    ...
    panel.querySelectorAll<HTMLElement>('.eq-slot.filled').forEach(el => {
  ...
  private renderInventory(player: PlayerData) {
    ...
    player.inventory.forEach(item => {
      ...
      if (item.type === 'equip') slot.style.borderColor = rarityColor;
      if (blocker) slot.classList.add('blocked');
      ...
      if (item.weaponType) lines.push(`ประเภท: ${WEAPON_PROFILES[item.weaponType].thaiName}`);
      if (item.reqLevel) lines.push(`ต้องการ Lv.${item.reqLevel}`);
      if (blocker) lines.push(`⛔ ${blocker}`);
      else if (item.type === 'equip') lines.push('🖱️ คลิกเพื่อสวมใส่');
      else if (item.type === 'usable') lines.push('🖱️ คลิกเพื่อใช้งาน');
  ...
  private buildShopWindow() {
    if (document.getElementById('shop-win')) return;
    ...
    win.querySelectorAll<HTMLElement>('.shop-tab').forEach(btn => {
  ...
  public openShop(npcId: string, shopId: string) {
  ...
  private renderShop() {
    ...
    if (!list || !this.shop || !player) return;
    const { shopId, npcId, tab } = this.shop;
    const shop = SHOPS[shopId];
    const merchant = player.job === 'Merchant';

    const title = document.getElementById('shop-title');
    if (title) title.textContent = shop.title;
    const zeny = document.getElementById('shop-zeny');
    if (zeny) zeny.textContent = `💰 ${player.zeny.toLocaleString()} Z${merchant ? ' · ส่วนลดพ่อค้า 10%' : ''}`;
    document.querySelectorAll<HTMLElement>('.shop-tab').forEach(b => b.classList.toggle('active', b.dataset.tab === tab));

    if (tab === 'buy') {
      list.innerHTML = shop.items.map(id => ITEM_DB[id]).filter((d): d is ItemDef => Boolean(d)).map(def => {
        const price = Math.ceil(def.price * (merchant ? 0.9 : 1));
        const blocker = def.type === 'equip' ? getEquipBlocker({ ...def, id: def.id, quantity: 1 } as InventoryItem, player.job, player.baseLevel) : null;
      ...
      list.querySelectorAll<HTMLButtonElement>('.shop-buy').forEach(btn => {
        btn.addEventListener('click', () => {
          const itemId = btn.dataset.item || '';
          const qtyInput = list.querySelector<HTMLInputElement>(`.shop-qty[data-item="${itemId}"]`);
      ...
      list.querySelectorAll<HTMLButtonElement>('.shop-sell').forEach(btn => {
        btn.addEventListener('click', () => {
          const itemId = btn.dataset.item || '';
          const qtyInput = list.querySelector<HTMLInputElement>(`.shop-qty[data-item="${itemId}"]`);
  ...
  private renderSkillTree(player: PlayerData) {
    const body = document.querySelector('#skills-win .ro-window-body') as HTMLElement | null;
    if (!body) return;
    const derived = computeDerived(player);
    const hotkeys: Record<string, string> = {};
    Object.entries(this.getHotbarSkills(player.job)).forEach(([slot, skill]) => {
      if (skill) hotkeys[skill] = slot.replace('slot-', '').toUpperCase();
    });
    ...
    body.querySelectorAll<HTMLButtonElement>('.skill-up').forEach(btn => {
  ...
  public startCooldown(skillId: string, ms: number) {
    if (ms < 300) return; // basic attacks are too quick to be worth a sweep
    const now = performance.now();
    this.cooldowns.set(skillId, { start: now, end: now + ms });
    if (!this.cooldownFrame) this.cooldownFrame = requestAnimationFrame(this.tickCooldowns);
  }
  ...
  private tickCooldowns = () => {
    ...
    Object.entries(bindings).forEach(([slotId, skillId]) => {
      const overlay = document.getElementById(slotId)?.querySelector('.cd-overlay') as HTMLElement | null;
      if (!overlay) return;
      ...
      if (text) text.textContent = remaining > 1000 ? `${Math.ceil(remaining / 1000)}` : (remaining / 1000).toFixed(1);
    });

    for (const [id, cd] of this.cooldowns) {
      if (now >= cd.end) this.cooldowns.delete(id);
    }
    this.cooldownFrame = this.cooldowns.size > 0 ? requestAnimationFrame(this.tickCooldowns) : 0;
    if (!this.cooldownFrame) {
      document.querySelectorAll<HTMLElement>('.cd-overlay').forEach(o => { o.style.display = 'none'; });
    }
  };
  ...
  private buildTargetFrame() {
    if (document.getElementById('target-frame')) return;
  ...
  public setTarget(target: { name: string; level: number; hp: number; maxHp: number; boss?: boolean } | null) {
    ...
    (document.getElementById('tf-name') as HTMLElement).textContent = target.name;
    (document.getElementById('tf-level') as HTMLElement).textContent = `Lv.${target.level}`;
    (document.getElementById('tf-fill') as HTMLElement).style.width = `${pct}%`;
    (document.getElementById('tf-hp') as HTMLElement).textContent = `${target.hp.toLocaleString()} / ${target.maxHp.toLocaleString()}`;
  }
  ...
  public showDeath(killerName: string, expLost: number) {
    const overlay = document.getElementById('death-overlay');
    const detail = document.getElementById('death-detail');
    const btn = document.getElementById('death-respawn') as HTMLButtonElement | null;
    if (!overlay || !detail || !btn) return;
  ...
  private buildToastLayer() {
    if (document.getElementById('toast-layer')) return;
  ...
  public toast(text: string, kind: 'info' | 'error' | 'success' = 'info') {
  ...
  private injectStyles() {
    if (document.getElementById('gameplay-ui-styles')) return;

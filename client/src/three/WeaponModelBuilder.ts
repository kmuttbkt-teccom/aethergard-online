...
import { InventoryItem, ItemRarity, WeaponType } from '../../../server/src/types.js';
...
const PALETTES: Record<ItemRarity, RarityPalette> = {
...
export class WeaponModelBuilder {
  ...
  public static create(weaponType: WeaponType, refine = 0, rarity: ItemRarity = 'common'): THREE.Group {
    ...
    switch (weaponType) {
      case 'dagger': weapon = this.dagger(metal, rune, accent, leather); break;
      case 'sword': weapon = this.sword(metal, rune, accent, leather, 1.0); break;
      case 'greatsword': weapon = this.sword(metal, rune, accent, leather, 1.45); break;
      case 'spear': weapon = this.spear(metal, rune, wood); break;
      case 'axe': weapon = this.axe(metal, rune, wood); break;
      case 'mace': weapon = this.mace(metal, rune, wood); break;
      case 'scythe': weapon = this.scythe(metal, rune, accent); break;
      case 'staff': weapon = this.staff(rune, accent, wood); break;
      case 'rod': weapon = this.rod(rune, accent); break;
      case 'bow': weapon = this.bow(wood, rune, accent); break;
      default: weapon = new THREE.Group(); break;
    }
    ...
    if (weaponType !== 'bow') weapon.rotation.x = Math.PI / 2;
    return weapon;
  }

  public static createForItem(item?: InventoryItem | null): THREE.Group | null {
    if (!item) return null;
  ...
  public static animateGlow(weaponRoot: THREE.Object3D, time: number) {
    const glow = weaponRoot.getObjectByName('refine_glow') as THREE.Sprite | undefined;
    if (glow) {
  ...
  private static dagger(metal: THREE.Material, rune: THREE.Material, accent: THREE.Material, grip: THREE.Material) {
  ...
  private static sword(metal: THREE.Material, rune: THREE.Material, accent: THREE.Material, grip: THREE.Material, scale: number) {
    const g = new THREE.Group();
  ...
  private static spear(metal: THREE.Material, rune: THREE.Material, wood: THREE.Material) {
    const g = new THREE.Group();
  ...
  private static axe(metal: THREE.Material, rune: THREE.Material, wood: THREE.Material) {
    const g = new THREE.Group();
  ...
  private static mace(metal: THREE.Material, rune: THREE.Material, wood: THREE.Material) {
    const g = new THREE.Group();
  ...
  private static scythe(metal: THREE.Material, rune: THREE.Material, accent: THREE.Material) {
    const g = new THREE.Group();
  ...
  private static staff(rune: THREE.Material, accent: THREE.Material, wood: THREE.Material) {
    const g = new THREE.Group();
  ...
  private static rod(rune: THREE.Material, accent: THREE.Material) {
    const g = new THREE.Group();
  ...
  private static bow(wood: THREE.Material, rune: THREE.Material, accent: THREE.Material) {
    const g = new THREE.Group();

import { Account, CharacterModel } from '../../../server/src/types.js';
import { escapeHtml } from './escape.js';

export interface AuthUICallbacks {
  onStartGame: (token: string, character: CharacterModel) => void;
}
...
export class AuthUI {
  ...
  private selectedCharacter: CharacterModel | null = null;
  private selectedSlotForCreate: number = 0;
  ...
  private updateActionButtons() {
    const playBtn = document.getElementById('btn-play-character') as HTMLButtonElement;
    const delBtn = document.getElementById('btn-delete-character') as HTMLButtonElement;
  ...
  private setupListeners() {
    ...
    document.getElementById('btn-login-guest')?.addEventListener('click', async () => {
      ...
      if (data.success) {
        ...
        if (data.guestSecret) localStorage.setItem('midgard_guest_secret', data.guestSecret);
    ...
    document.getElementById('btn-submit-login')?.addEventListener('click', async () => {
      ...
      if (data.success) {
        ...
        if (errEl) errEl.textContent = data.error || 'เข้าสู่ระบบล้มเหลว';
      }
    });
    ...
    document.getElementById('btn-submit-register')?.addEventListener('click', async () => {
      ...
      const name = (document.getElementById('reg-display-name') as HTMLInputElement)?.value.trim();
      ...
      if (data.success) {
        ...
        if (errEl) errEl.textContent = data.error || 'ลงทะเบียนล้มเหลว';
      }
    });
    ...
    document.getElementById('btn-delete-character')?.addEventListener('click', async () => {
      if (!this.selectedCharacter || !this.token) return;
    ...
    document.getElementById('btn-confirm-create-char')?.addEventListener('click', async () => {
      ...
      const gender = (document.querySelector('input[name="char-gender"]:checked') as HTMLInputElement)?.value as 'male' | 'female' || 'male';
      const hairStyle = parseInt((document.getElementById('new-char-hairstyle') as HTMLSelectElement)?.value || '0');
      const hairColor = (document.getElementById('new-char-haircolor') as HTMLSelectElement)?.value || '#9a031e';
      ...
      if (data.success) {
        ...
        if (errEl) errEl.textContent = data.error || 'สร้างตัวละครล้มเหลว';
      }
    });
  }

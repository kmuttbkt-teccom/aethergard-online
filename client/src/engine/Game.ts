...
import { PlayerData, MonsterData, DropItemData, Stats, JobClass } from '../../../server/src/types.js';
...
export class Game {
  ...
  public selfPlayer: ClientPlayer | null = null;
  public remotePlayers: Map<string, ClientPlayer> = new Map();
  public monsters: Map<string, ClientMonster> = new Map();
  public drops: Map<string, ClientDropItem> = new Map();
  ...
  public start(token: string = '', characterId: string = '') {
  ...
  private update(dt: number) {
    ...
    // UI Window Hotkeys
    if (this.input.isJustPressed('KeyS')) this.ui.toggleWindow('status-win');
    if (this.input.isJustPressed('KeyI')) this.ui.toggleWindow('inventory-win');
    if (this.input.isJustPressed('KeyK')) this.ui.toggleWindow('skills-win');
    if (this.input.isJustPressed('KeyJ')) this.ui.toggleWindow('job-win');
    if (this.input.isJustPressed('KeyH')) this.ui.toggleWindow('help-win');
    ...
    this.ui.renderMiniMap(
      this.selfPlayer.data,
      Array.from(this.remotePlayers.values()).map(p => p.data),
      Array.from(this.monsters.values()).map(m => m.data),
      this.map.width,
      this.map.height
    );
  }

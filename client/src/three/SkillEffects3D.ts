...
import { WeaponType } from '../../../server/src/types.js';
import { SKILL_DB, SkillElement, WEAPON_PROFILES } from '../../../server/src/GameData.js';
import { SkillEffect3DManager } from './SkillEffect3DManager.js';

export const ELEMENT_COLORS: Record<SkillElement, number> = {
  neutral: 0xffffff,
  fire: 0xff5a1f,
  ice: 0x7dd3fc,
  wind: 0x6ee7b7,
...
export class SkillEffects3D {
  ...
  public advancedEffects: SkillEffect3DManager;
  ...
  public playSkill(
    ...
    switch (skillId) {
      case 'NORMAL': {
        ...
        if (kind === 'ranged' && target) this.projectile(caster, target, 'arrow', 0xf8f9fa);
        else if (kind === 'magic' && target) this.projectile(caster, target, 'orb', weaponType === 'rod' ? 0xffd166 : 0x7dd3fc);
        else this.slashArc(caster, rotY, 0xffffff, 1.0);
        break;
      }
      case 'BASH':
        this.slashArc(caster, rotY, 0xff7b00, 1.35);
        if (target) this.impact(target, 0xff7b00, 1.2);
        break;
      case 'RADIANT_SLASH':
      ...
      case 'FROST_NOVA':
        ...
        break;
      case 'GALE_ARROW':
        if (target) this.projectile(caster, target, 'arrow', color, 1.6);
        break;
      case 'RAIN_OF_LIGHT':
      ...
      case 'METEOR_STORM':
        for (let i = 0; i < 4; i++) {
          const offset = new THREE.Vector3(
            center.x + (Math.random() - 0.5) * radius * 1.5,
            center.y,
      ...
      case 'SHADOW_CLEAVE':
        ...
        if (target) this.impact(target, 0xd90429, 1.6);
        break;
      case 'TITAN_SMASH':
  ...
  public slashArc(pos: THREE.Vector3, rotY: number, color: number, scale = 1) {
  ...
  public impact(pos: THREE.Vector3, color: number, scale = 1) {
    const d = new Disposables();
    ...
    const shards: Array<{ mesh: THREE.Mesh; vel: THREE.Vector3 }> = [];
    for (let i = 0; i < 10; i++) {
  ...
  public dragonFlameJet(from: THREE.Vector3, to: THREE.Vector3) {
    const d = new Disposables();
    const group = new THREE.Group();
  ...
  public projectile(from: THREE.Vector3, to: THREE.Vector3, kind: 'arrow' | 'orb', color: number, scale = 1, onArrive?: () => void) {
    const d = new Disposables();
    const group = new THREE.Group();
  ...
  public groundRing(pos: THREE.Vector3, radius: number, color: number, duration = 0.5, filled = false) {
    const d = new Disposables();
    ...
    this.effects.push({
      update: (dt) => {
        t += dt;
        const k = t / duration;
        if (!filled) mesh.scale.setScalar(0.3 + Math.min(1, k * 2) * 0.7);
        mesh.rotation.y += dt;
  ...
  public spinSlash(pos: THREE.Vector3, color: number, radius: number) {
    for (let i = 0; i < 3; i++) {
      this.slashArc(pos, (i / 3) * Math.PI * 2, color, Math.max(1, radius / 2.2));
    }
  }

  public dome(pos: THREE.Vector3, color: number) {
    const d = new Disposables();
  ...
  public healPillar(pos: THREE.Vector3, color: number, scale = 1) {
    const d = new Disposables();
    const group = new THREE.Group();
  ...
  public meteor(center: THREE.Vector3, radius: number, color: number) {
    const d = new Disposables();
  ...
  public nova(center: THREE.Vector3, radius: number, color: number) {
    const d = new Disposables();
    const group = new THREE.Group();
  ...
  public lightRain(center: THREE.Vector3, radius: number, color: number) {
  ...
  private fallingBolt(from: THREE.Vector3, to: THREE.Vector3, color: number) {
    const d = new Disposables();
    ...
    this.effects.push({
      update: (dt) => {
        ...
        if (k >= 1) this.impact(end, color, 0.5);
        return k < 1;
      },
  ...
  public smoke(pos: THREE.Vector3, color: number) {
    const d = new Disposables();
    ...
    const puffs: Array<{ m: THREE.Mesh; v: THREE.Vector3 }> = [];
    const group = new THREE.Group();
  ...
  public bladeDance(center: THREE.Vector3, radius: number, color: number) {
    const d = new Disposables();
    const group = new THREE.Group();
  ...
  public coinBurst(center: THREE.Vector3, radius: number) {
    const d = new Disposables();
    const group = new THREE.Group();
    ...
    const coins: Array<{ m: THREE.Mesh; v: THREE.Vector3 }> = [];
    for (let i = 0; i < 16; i++) {
      const m = new THREE.Mesh(geo, mat);
    ...
    this.effects.push({
      update: (dt) => {
        ...
        coins.forEach(c => {
          ...
          if (c.m.position.y < -0.7) c.m.position.y = -0.7;
        });
        return t < 0.6;
      },
      dispose: () => {
  ...
  public vortex(center: THREE.Vector3, radius: number, color: number) {
    const d = new Disposables();
  ...
  public floatingText(pos: THREE.Vector3, text: string, fill: string, stroke: string, big = false) {
    const d = new Disposables();
  ...
  public floatingDamage(pos: THREE.Vector3, amount: number, isCrit = false) {
